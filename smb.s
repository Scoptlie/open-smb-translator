; Target assembler: cc65 v2.18.0 [--target none -C SuperMarioBros_cc65.cfg]
;*******************************************************************************
...
;*******************************************************************************
;* Project created by Andy McFadden, using 6502bench SourceGen v1.8.  This is  *
;* a straight conversion from the original text listing to SourceGen format.   *
...
Misc_State = $2a
PlayerFacingDir = $33
DestinationPageLoc = $34 ;2 defs for $34
...
BlooperMoveSpeed = $58 ;11 defs for $58
...
Block_X_Speed = $60
Misc_X_Speed = $64
Player_PageLoc = $6d ;2 defs for $6d
...
Misc_PageLoc = $7a
Bubble_PageLoc = $83
Player_X_Position = $86 ;2 defs for $86
...
BlooperMoveCounter = $a0 ;7 defs for $a0
Enemy_Y_Speed = $a0
...
Block_Y_Speed = $a8
Misc_Y_Speed = $ac
Player_Y_HighPos = $b5 ;2 defs for $b5
...
Misc_Y_HighPos = $c2
Bubble_Y_HighPos = $cb
Player_Y_Position = $ce ;2 defs for $ce
...
HammerThrowingTimer = $03a2 ;2 defs for $03a2
PlatformCollisionFlag = $03a2
Player_Rel_XPos = $03ad ;2 defs for $03ad
...
Block_Rel_XPos = $03b1
Misc_Rel_XPos = $03b3
Player_Rel_YPos = $03b8 ;2 defs for $03b8
...
Player_OffscreenBits = $03d0 ;2 defs for $03d0
...
Cannon_PageLoc = $046b ;2 defs for $046b
Whirlpool_PageLoc = $046b
Cannon_X_Position = $0471 ;2 defs for $0471
Whirlpool_LeftExtent = $0471
Cannon_Y_Position = $0477 ;2 defs for $0477
Whirlpool_Length = $0477
Cannon_Timer = $047d ;2 defs for $047d
Whirlpool_Flag = $047d
BowserHitPoints = $0483
StompChainCounter = $0484
Player_CollisionBits = $0490
Enemy_CollisionBits = $0491
Player_BoundBoxCtrl = $0499 ;2 defs for $0499
...
Alt_SprDataOffset = $06ec ;2 defs for $06ec
...
; 
    ...
    .byte "NES"
    .byte $1a ;Ctrl+Z
    .byte $02 ;PRG ROM is 2 x 16KB
    .byte $01 ;CHR ROM is 1 x 8KB
...
ColdBoot:
    ...
    sta DMC_RAW ;reset delta counter load register
    sta OperMode ;reset primary mode of operation
    lda #$a5 ;set warm boot flag
    sta WarmBootValidation
    sta PseudoRandomBitReg ;set seed for pseudorandom register
    lda #%00001111
    sta SND_CHN ;enable all sound channels except dmc
    lda #%00000110
...
VRAM_AddrTable_Low:
    .byte <VRAM_Buffer1
    .byte <WaterPaletteData
    .byte <GroundPaletteData
    .byte <UndergroundPaletteData
    .byte <CastlePaletteData
    .byte <VRAM_Buffer1_Offset
    .byte <VRAM_Buffer2
    .byte <VRAM_Buffer2
    .byte <BowserPaletteData
    .byte <DaySnowPaletteData
    .byte <NightSnowPaletteData
    .byte <MushroomPaletteData
    .byte <MarioThanksMessage
    .byte <LuigiThanksMessage
    .byte <MushroomRetainerSaved
    .byte <PrincessSaved1
    .byte <PrincessSaved2
    .byte <WorldSelectMessage1
...
DecTimersLoop:
    lda SelectTimer,x ;check current timer
    beq SkipExpTimer ;if current timer expired, branch to skip,
    dec SelectTimer,x ; otherwise decrement the current timer
SkipExpTimer:
...
PauseSkip:
    ldx #$00
    ldy #$07
    lda PseudoRandomBitReg ;get first memory locaion of LSFR bytes
...
Sprite0Hit:
    ...
    and #%01000000
    beq Sprite0Hit
    ldy #$14 ;small delay, to wait until we hit orizontal blank time
HBlankDelay:
    dey
    bne HBlankDelay
SkipSprite0:
...
GameMenuRoutine:
    ...
    cmp #Start_Button
    beq StartGame
    cmp #$90 ;||check to see if A + start was pressed
...
; -----------------------------------------------------------------------------
DemoActionData:
    .byte $01,$80,$02,$81,$41,$80,$01,$42,$c2,$02,$80,$41,$c1,$41,$c1,$01
    .byte $c1,$01,$02,$80,$00
DemoTimingData:
    .byte $9b,$10,$18,$05,$2c,$20,$24,$15,$5a,$10,$20,$28,$30,$20,$10,$80
    .byte $20,$30,$30,$01,$ff,$00
...
DontWalk:
    ...
    jsr AutoControlPlayer ; use A to mvoe player to the right or not
...
ScoreUpdateData:
    .byte $ff ;dummy
    .byte $41,$42,$44,$45,$48,$31,$32,$34,$35,$38,$00
...
SetupIntermediate:
    ...
    jmp IncSubtask ; then move into the next task
...
OutputTScr:
    ...
    iny
    bne ChkHiByte ;if not past 256 bytesw, do not increment
    inc $01 ;otherwise increment high byte of indirect
ChkHiByte:
    lda $01 ;heck high byte?
    cmp #$04 ;at $0400?
...
ClearBuffersDrawIcon:
    ...
    bne IncModeTask_B ;if not title screen mnode, leave
    ldx #$00 ;otherwise, clear buffer space
TScrClear:
    sta VRAM_Buffer1-1,x
    sta VRAM_Buffer1+$ff,x
    dex
    bne TScrClear
...
GameTextOffsets:
    .byte $00,$00,$27,$27,$46,$4e,$59,$61,$6e,$6e

WriteGameText:
    pha ;save text number to stack
    asl
    tay ;multiply by 2 and use as offset
    cpy #$04 ;if set to do top status bar on world/lives display,
...
PutLives:
    ...
    ldy LevelNumber
    iny
    sty VRAM_Buffer1+$15 ;we're done here
    rts
...
WarpNumLoop:
    ...
    sta VRAM_Buffer1+$1b,y ; placeholders from earlier
    inx
...
RenderAreaGraphics:
    ...
    tax
DrawMTLoop:
    stx $01 ;tore init value of 0 or incremented offset for buffer
    ...
    rol $03 ;this in d1-d0, for upper left square
    rol $03
    jmp SetAttrib
...
LLeft:
    ...
    lsr $03 ;this in d5-d4 for lower left square
NextMTRow:
    inc $04 ;move onto next attribute row
SetAttrib:
    lda AttributeBuffer,y ;get previously saved bits from before
    ora $03 ; if any, and put new bits, if any
    sta AttributeBuffer,y ; onto the old, and store
...
PutBlockMetatile:
    ...
    bcc SaveHAddr ;if not, use current high byte
...
RemBridge:
    ...
    lda #$00
    sta VRAM_Buffer1+$09,y ;put null terminator at end
...
Palette0_MTiles:
    ...
    .byte $b8,$ba,$b9,$bb ;tall tree, top half
...
PortLoop:
    ...
    lda JOY1,x ;read current bit on joypad port
    ...
    sta SavedJoypad1Bits,x ;save controller status here always
    pha
    ...
    sta SavedJoypad1Bits,x ; or start bits and leave
    rts
...
SetupNums:
    ...
    ldx $02
DigitPLoop:
    lda TopScoreDisplay,y ;write digits to the buffer
    sta VRAM_Buffer1+3,x
    inx
    iny
...
CarryOne:
    ...
    sbc #$0a ; proper BCD number, then increment the digit
...
TopScoreCheck:
    ...
    sec
GetScoreDiff:
    lda ScoreAndCoinDisplay,x ;subtract each player digit from each high score digit
    ...
    iny
CopyScore:
    lda ScoreAndCoinDisplay,x ;store player's score digits into high score memory area
...
; -----------------------------------------------------------------------------
DefaultSprOffsets:
    .byte $04,$30,$48,$60,$78,$90,$a8,$c0,$d8,$e8,$24,$f8,$fc,$28,$2c
Sprite0Data:
    .byte $18,$ff,$23,$58
...
PlayerStarting_Y_Pos:
    .byte $00,$20,$b0,$50,$00,$00,$b0,$b0,$f0
...
DoNothing1:
    ...
    sta unused_06c9 ; not used anywhere in the program
DoNothing2:
    rts
...
BSceneDataOffsets:
    .byte $00,$30,$60
BackSceneryData:
    .byte $93,$00,$00,$11,$12,$12,$13,$00,$00,$51,$52,$53,$00,$00,$00,$00 ;clouds
    .byte $00,$00,$01,$02,$02,$03,$00,$00,$00,$00,$00,$00,$91,$92,$93,$00
    .byte $00,$00,$00,$51,$52,$53,$41,$42,$43,$00,$00,$00,$00,$00,$91,$92
    .byte $97,$87,$88,$89,$99,$00,$00,$00,$11,$12,$13,$a4,$a5,$a5,$a5,$a6 ;mountains and bushes
    .byte $97,$98,$99,$01,$02,$03,$00,$a4,$a5,$a6,$00,$11,$12,$12,$12,$13
    .byte $00,$00,$00,$00,$01,$02,$02,$03,$00,$a4,$a5,$a5,$a6,$00,$00,$00
    .byte $11,$12,$12,$13,$00,$00,$00,$00,$00,$00,$00,$9c,$00,$8b,$aa,$aa ;trees and fences
    .byte $aa,$aa,$11,$12,$13,$8b,$00,$9c,$9c,$00,$00,$01,$02,$03,$11,$12
    .byte $12,$13,$00,$00,$00,$00,$aa,$aa,$9c,$aa,$00,$8b,$00,$01,$02,$03
...
FSceneDataOffsets:
    .byte $00,$0d,$1a
ForeSceneryData:
    .byte $86,$87,$87,$87,$87,$87,$87,$87,$87,$87,$87,$69,$69 ;in water
    .byte $00,$00,$00,$00,$00,$45,$47,$47,$47,$47,$47,$00,$00 ;wall
    .byte $00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$86,$87 ;over water
TerrainMetatiles:
    .byte $69,$54,$52,$62
TerrainRenderBits:
    .dbyt %0000000000000000 ;no ceiling or floor
    .dbyt %0000000000011000 ;no ceiling, floor 2
    .dbyt %0000000100011000 ;ceiling 1, floor 2
    .dbyt %0000011100011000 ;ceiling 3, floor 2
    .dbyt %0000111100011000 ;ceiling 4, floor 2
    .dbyt %1111111100011000 ;ceiling 8, floor 2
    .dbyt %0000000100011111 ;ceiling 1, floor 5
    .dbyt %0000011100011111 ;ceiling 3, floor 5
    .dbyt %0000111100011111 ;ceiling 4, floor 5
    .dbyt %1000000100011111 ;ceiling 1, floor 6
    .dbyt %0000000100000000 ;ceiling 1, no floor
    .dbyt %1000111100011111 ;ceiling 4, floor 6
    .dbyt %1111000100011111 ;ceiling 1, floor 9
    .dbyt %1111100100011000 ;ceiling 1, middle 5, floor 2
    .dbyt %1111000100011000 ;ceiling 1, middle 4, floor 2
    .dbyt %1111111100011111 ;completely solid top to bottom
...
NormObj:
    ...
    bne StarAObj ;if set, branch to render object, else leave
LeavePar:
    rts
...
; large objects (rows $00-$0b or 00-11, d6-d4 set)
    .word VerticalPipe
    .word AreaStyleObject ;used by warp pipes
    .word RowOfBricks
...
; -----------------------------------------------------------------------------
FrenzyIDData:
    .byte FlyCheepCheepFrenzy
    .byte BBill_CCheep_Frenzy
    .byte Stop_Frenzy
...
PlayerStop:
    ...
    sty MetatileBuffer+$0a ;this is only done if we're on the second column
ExitCastle:
    rts
...
; 
WorldAddrOffsets:
    .byte $00,$05,$0a,$0e,$13,$17,$1b,$20 ;||offsets from AreaAddrOffsets
AreaAddrOffsets:
    .byte $25,$29,$c0,$26,$60 ;World1Areas
World2Areas:
    .byte $28,$29,$01,$27,$62
World3Areas:
    .byte $24,$35,$20,$63
...
EnemyDataAddrLow:
    ...
    .byte <E_GroundArea21
    .byte <E_GroundArea22
    .byte <E_UndergroundArea1
    .byte <E_UndergroundArea2
    .byte <E_UndergroundArea3
    .byte <E_WaterArea1
    .byte <E_WaterArea2
    .byte <E_WaterArea3
EnemyDataAddrHigh:
...
AreaDataAddrLow:
    ...
    .byte <L_GroundArea21
    .byte <L_GroundArea22
    .byte <L_UndergroundArea1
    .byte <L_UndergroundArea2
    .byte <L_UndergroundArea3
    .byte <L_CastleArea1
    .byte <L_CastleArea2
    .byte <L_CastleArea3
    .byte <L_CastleArea4
...
; level 1-4/6-4
E_CastleArea1:
    .byte $76,$dd,$bb,$4c,$ea,$1d,$1b,$cc,$56,$5d,$16,$9d,$c6,$1d,$36,$9d
    .byte $c9,$1d,$04,$db,$49,$1d,$84,$1b,$c9,$5d,$88,$95,$0f,$08,$30,$4c
    .byte $78,$2d,$a6,$28,$90,$b5
    .byte $ff
; level 4-4
E_CastleArea2:
    .byte $0f,$03,$56,$1b,$c9,$1b,$0f,$07,$36,$1b,$aa,$1b,$48,$95,$0f,$0a
    .byte $2a,$1b,$5b,$0c,$78,$2d,$90,$b5
    .byte $ff
; level 2-4/5-4
E_CastleArea3:
    .byte $0b,$8c,$4b,$4c,$77,$5f,$eb,$0c,$bd,$db,$19,$9d,$75,$1d,$7d,$5b
    .byte $d9,$1d,$3d,$dd,$99,$1d,$26,$9d,$5a,$2b,$8a,$2c,$ca,$1b,$20,$95
    .byte $7b,$5c,$db,$4c,$1b,$cc,$3b,$cc,$78,$2d,$a6,$28,$90,$b5
    .byte $ff
; level 3-4
E_CastleArea4:
    .byte $0b,$8c,$3b,$1d,$8b,$1d,$ab,$0c,$db,$1d,$0f,$03,$65,$1d,$6b,$1b
    .byte $05,$9d,$0b,$1b,$05,$9b,$0b,$1d,$8b,$0c,$1b,$8c,$70,$15,$7b,$0c
    .byte $db,$0c,$0f,$08,$78,$2d,$a6,$28,$90,$b5
    .byte $ff
; level 7-4
E_CastleArea5:
...
; level 8-4
E_CastleArea6:
    .byte $0f,$03,$8e,$65,$e1,$bb,$38,$6d,$a8,$3e,$e5,$e7,$0f,$08,$0b,$02
    .byte $2b,$02,$5e,$65,$e1,$bb,$0e,$db,$0e,$bb,$8e,$db,$0e,$fe,$65,$ec
    .byte $0f,$0d,$4e,$65,$e1,$0f,$0e,$4e,$02,$e0,$0f,$10,$fe,$e5,$e1,$1b
    .byte $85,$7b,$0c,$5b,$95,$78,$2d,$90,$b5
    .byte $ff
; level 3-3
E_GroundArea1:
    .byte $a5,$86,$e4,$28,$18,$a8,$45,$83,$69,$03,$c6,$29,$9b,$83,$16,$a4
    .byte $88,$24,$e9,$28,$05,$a8,$7b,$28,$24,$8f,$c8,$03,$e8,$03,$46,$a8
    .byte $85,$24,$c8,$24
    .byte $ff
; level 8-3
E_GroundArea2:
    .byte $eb,$8e,$0f,$03,$fb,$05,$17,$85,$db,$8e,$0f,$07,$57,$05,$7b,$05
    .byte $9b,$80,$2b,$85,$fb,$05,$0f,$0b,$1b,$05,$9b,$05
    .byte $ff
; level 4-1
E_GroundArea3:
    .byte $2e,$c2,$66,$e2,$11,$0f,$07,$02,$11,$0f,$0c,$12,$11
    .byte $ff
; level 6-2
E_GroundArea4:
    .byte $0e,$c2,$a8,$ab,$00,$bb,$8e,$6b,$82,$de,$00,$a0,$33,$86,$43,$06
    .byte $3e,$b4,$a0,$cb,$02,$0f,$07,$7e,$42,$a6,$83,$02,$0f,$0a,$3b,$02
    .byte $cb,$37,$0f,$0c,$e3,$0e
    .byte $ff
; level 3-1
E_GroundArea5:
...
E_GroundArea6:
    ...
    .byte $b7,$bb,$00,$5b,$b7,$fb,$37,$3b,$b7,$0f,$0b,$1b,$37
    .byte $ff
; level 1-3/5-3
E_GroundArea7:
    .byte $2b,$d7,$e3,$03,$c2,$86,$e2,$06,$76,$a5,$a3,$8f,$03,$86,$2b,$57
    .byte $68,$28,$e9,$28,$e5,$83,$24,$8f,$36,$a8,$5b,$03
    .byte $ff
; level 2-3/7-3
E_GroundArea8:
    .byte $0f,$02,$78,$40,$48,$ce,$f8,$c3,$f8,$c3,$0f,$07,$7b,$43,$c6,$d0
    .byte $0f,$8a,$c8,$50
    .byte $ff
; level 2-1
E_GroundArea9:
    .byte $85,$86,$0b,$80,$1b,$00,$db,$37,$77,$80,$eb,$37,$fe,$2b,$20,$2b
    .byte $80,$7b,$38,$ab,$b8,$77,$86,$fe,$42,$20,$49,$86,$8b,$06,$9b,$80
...
E_GroundArea11:
    ...
    .byte $a0,$b8,$c0,$b8,$fb,$00,$a0,$b8,$30,$bb,$ee,$42,$88,$0f,$0b,$2b
    .byte $0e,$67,$0e
    .byte $ff
...
E_GroundArea13:
    ...
    .byte $4b,$83,$e3,$03,$05,$a4,$89,$24,$b5,$24,$09,$a4,$65,$24,$c9,$24
    .byte $0f,$08,$85,$25
    .byte $ff
; level 6-3
E_GroundArea14:
    .byte $cd,$a5,$b5,$a8,$07,$a8,$76,$28,$cc,$25,$65,$a4,$a9,$24,$e5,$24
    .byte $19,$a4,$0f,$07,$95,$28,$e6,$24,$19,$a4,$d7,$29,$16,$a9,$58,$29
    .byte $97,$29
    .byte $ff
; level 6-1
E_GroundArea15:
...
; level 8-1
E_GroundArea17:
    .byte $2b,$82,$ab,$38,$de,$42,$e2,$1b,$b8,$eb,$3b,$db,$80,$8b,$b8,$1b
    .byte $82,$fb,$b8,$7b,$80,$fb,$3c,$5b,$bc,$7b,$b8,$1b,$8e,$cb,$0e,$1b
    .byte $8e,$0f,$0d,$2b,$3b,$bb,$b8,$eb,$82,$4b,$b8,$bb,$38,$3b,$b7,$bb
    .byte $02,$0f,$13,$1b,$00,$cb,$80,$6b,$bc
    .byte $ff
; level 5-2
E_GroundArea18:
    .byte $7b,$80,$ae,$00,$80,$8b,$8e,$e8,$05,$f9,$86,$17,$86,$16,$85,$4e
    .byte $2b,$80,$ab,$8e,$87,$85,$c3,$05,$8b,$82,$9b,$02,$ab,$02,$bb,$86
    .byte $cb,$06,$d3,$03,$3b,$8e,$6b,$0e,$a7,$8e
    .byte $ff
; level 8-2
E_GroundArea19:
    .byte $29,$8e,$52,$11,$83,$0e,$0f,$03,$9b,$0e,$2b,$8e,$5b,$0e,$cb,$8e
    .byte $fb,$0e,$fb,$82,$9b,$82,$bb,$02,$fe,$42,$e8,$bb,$8e,$0f,$0a,$ab
    .byte $0e,$cb,$0e,$f9,$0e,$88,$86,$a6,$06,$db,$02,$b6,$8e
    .byte $ff
; level 7-1
E_GroundArea20:
    .byte $ab,$ce,$de,$42,$c0,$cb,$ce,$5b,$8e,$1b,$ce,$4b,$85,$67,$45,$0f
    .byte $07,$2b,$00,$7b,$85,$97,$05,$0f,$0a,$92,$02
    .byte $ff
...
E_GroundArea22:
    ...
    .byte $cb,$8e,$fb,$80,$ab,$b8,$6b,$80,$fb,$3c,$9b,$bb,$5b,$bc,$fb,$00
    .byte $6b,$b8,$fb,$38
    .byte $ff
; level 1-2
E_UndergroundArea1:
    ...
    .byte $27,$2b,$83,$a1,$26,$a9,$26,$ee,$25,$0b,$27,$b4
    .byte $ff
; level 4-2
E_UndergroundArea2:
    .byte $0f,$02,$1e,$2f,$60,$e0,$3a,$a5,$a7,$db,$80,$3b,$82,$8b,$02,$fe
    .byte $42,$68,$70,$bb,$25,$a7,$2c,$27,$b2,$26,$b9,$26,$9b,$80,$a8,$82
    .byte $b5,$27,$bc,$27,$b0,$bb,$3b,$82,$87,$34,$ee,$25,$6b
    .byte $ff
; underground bonus rooms area used in many levels
E_UndergroundArea3:
    .byte $1e,$a5,$0a,$2e,$28,$27,$2e,$33,$c7,$0f,$03,$1e,$40,$07,$2e,$30
    .byte $e7,$0f,$05,$1e,$24,$44,$0f,$07,$1e,$22,$6a,$2e,$23,$ab,$0f,$09
    .byte $1e,$41,$68,$1e,$2a,$8a,$2e,$23,$a2,$2e,$32,$ea
    .byte $ff
...
; level 2-2/7-2
E_WaterArea2:
    .byte $0f,$01,$2e,$25,$2b,$2e,$25,$4b,$4e,$25,$cb,$6b,$07,$97,$47,$e9
    .byte $87,$47,$c7,$7a,$07,$d6,$c7,$78,$07,$38,$87,$ab,$47,$e3,$07,$9b
    .byte $87,$0f,$09,$68,$47,$db,$c7,$3b,$c7
    .byte $ff
; water area used in level 8-4
E_WaterArea3:
    .byte $47,$9b,$cb,$07,$fa,$1d,$86,$9b,$3a,$87,$56,$07,$88,$1b,$07,$9d
    .byte $2e,$65,$f0
    .byte $ff
; 
; AREA OBJECT DATA
...
L_CastleArea1:
    .byte $9b,$07
    .byte $05,$32,$06,$33,$07,$34,$ce,$03,$dc,$51,$ee,$07,$73,$e0,$74,$0a
    .byte $7e,$06,$9e,$0a,$ce,$06,$e4,$00,$e8,$0a,$fe,$0a,$2e,$89,$4e,$0b
    .byte $54,$0a,$14,$8a,$c4,$0a,$34,$8a,$7e,$06,$c7,$0a,$01,$e0,$02,$0a
    .byte $47,$0a,$81,$60,$82,$0a,$c7,$0a,$0e,$87,$7e,$02,$a7,$02,$b3,$02
    .byte $d7,$02,$e3,$02,$07,$82,$13,$02,$3e,$06,$7e,$02,$ae,$07,$fe,$0a
    .byte $0d,$c4,$cd,$43,$ce,$09,$de,$0b,$dd,$42,$fe,$02,$5d,$c7
    .byte $fd
; level 4-4
L_CastleArea2:
    .byte $5b,$07
    .byte $05,$32,$06,$33,$07,$34,$5e,$0a,$68,$64,$98,$64,$a8,$64,$ce,$06
    .byte $fe,$02,$0d,$01,$1e,$0e,$7e,$02,$94,$63,$b4,$63,$d4,$63,$f4,$63
    .byte $14,$e3,$2e,$0e,$5e,$02,$64,$35,$88,$72,$be,$0e,$0d,$04,$ae,$02
    .byte $ce,$08,$cd,$4b,$fe,$02,$0d,$05,$68,$31,$7e,$0a,$96,$31,$a9,$63
    .byte $a8,$33,$d5,$30,$ee,$02,$e6,$62,$f4,$61,$04,$b1,$08,$3f,$44,$33
    .byte $94,$63,$a4,$31,$e4,$31,$04,$bf,$08,$3f,$04,$bf,$08,$3f,$cd,$4b
    .byte $03,$e4,$0e,$03,$2e,$01,$7e,$06,$be,$02,$de,$06,$fe,$0a,$0d,$c4
    .byte $cd,$43,$ce,$09,$de,$0b,$dd,$42,$fe,$02,$5d,$c7
    .byte $fd
; level 2-4/5-4
L_CastleArea3:
    .byte $9b,$07
    .byte $05,$32,$06,$33,$07,$34,$fe,$00,$27,$b1,$65,$32,$75,$0a,$71,$00
    .byte $b7,$31,$08,$e4,$18,$64,$1e,$04,$57,$3b,$bb,$0a,$17,$8a,$27,$3a
    .byte $73,$0a,$7b,$0a,$d7,$0a,$e7,$3a,$3b,$8a,$97,$0a,$fe,$08,$24,$8a
    .byte $2e,$00,$3e,$40,$38,$64,$6f,$00,$9f,$00,$be,$43,$c8,$0a,$c9,$63
    .byte $ce,$07,$fe,$07,$2e,$81,$66,$42,$6a,$42,$79,$0a,$be,$00,$c8,$64
    .byte $f8,$64,$08,$e4,$2e,$07,$7e,$03,$9e,$07,$be,$03,$de,$07,$fe,$0a
    .byte $03,$a5,$0d,$44,$cd,$43,$ce,$09,$dd,$42,$de,$0b,$fe,$02,$5d,$c7
    .byte $fd
; level 3-4
L_CastleArea4:
    .byte $9b,$07
    .byte $05,$32,$06,$33,$07,$34,$fe,$06,$0c,$81,$39,$0a,$5c,$01,$89,$0a
    .byte $ac,$01,$d9,$0a,$fc,$01,$2e,$83,$a7,$01,$b7,$00,$c7,$01,$de,$0a
    .byte $fe,$02,$4e,$83,$5a,$32,$63,$0a,$69,$0a,$7e,$02,$ee,$03,$fa,$32
    .byte $03,$8a,$09,$0a,$1e,$02,$ee,$03,$fa,$32,$03,$8a,$09,$0a,$14,$42
    .byte $1e,$02,$7e,$0a,$9e,$07,$fe,$0a,$2e,$86,$5e,$0a,$8e,$06,$be,$0a
    .byte $ee,$07,$3e,$83,$5e,$07,$fe,$0a,$0d,$c4,$41,$52,$51,$52,$cd,$43
    .byte $ce,$09,$de,$0b,$dd,$42,$fe,$02,$5d,$c7
    .byte $fd
; level 7-4
L_CastleArea5:
    .byte $5b,$07
    .byte $05,$32,$06,$33,$07,$34,$fe,$0a,$ae,$86,$be,$07,$fe,$02,$0d,$02
    .byte $27,$32,$46,$61,$55,$62,$5e,$0e,$1e,$82,$68,$3c,$74,$3a,$7d,$4b
    .byte $5e,$8e,$7d,$4b,$7e,$82,$84,$62,$94,$61,$a4,$31,$bd,$4b,$ce,$06
    .byte $fe,$02,$0d,$06,$34,$31,$3e,$0a,$64,$32,$75,$0a,$7b,$61,$a4,$33
    .byte $ae,$02,$de,$0e,$3e,$82,$64,$32,$78,$32,$b4,$36,$c8,$36,$dd,$4b
    .byte $44,$b2,$58,$32,$94,$63,$a4,$3e,$ba,$30,$c9,$61,$ce,$06,$dd,$4b
    .byte $ce,$86,$dd,$4b,$fe,$02,$2e,$86,$5e,$02,$7e,$06,$fe,$02,$1e,$86
    .byte $3e,$02,$5e,$06,$7e,$02,$9e,$06,$fe,$0a,$0d,$c4,$cd,$43,$ce,$09
    .byte $de,$0b,$dd,$42,$fe,$02,$5d,$c7
    .byte $fd
; level 8-4
L_CastleArea6:
    .byte $5b,$06
    .byte $05,$32,$06,$33,$07,$34,$5e,$0a,$ae,$02,$0d,$01,$39,$73,$0d,$03
    .byte $39,$7b,$4d,$4b,$de,$06,$1e,$8a,$ae,$06,$c4,$33,$16,$fe,$a5,$77
    .byte $fe,$02,$fe,$82,$0d,$07,$39,$73,$a8,$74,$ed,$4b,$49,$fb,$e8,$74
    .byte $fe,$0a,$2e,$82,$67,$02,$84,$7a,$87,$31,$0d,$0b,$fe,$02,$0d,$0c
    .byte $39,$73,$5e,$06,$c6,$76,$45,$ff,$be,$0a,$dd,$48,$fe,$06,$3d,$cb
    .byte $46,$7e,$ad,$4a,$fe,$82,$39,$f3,$a9,$7b,$4e,$8a,$9e,$07,$fe,$0a
    .byte $0d,$c4,$cd,$43,$ce,$09,$de,$0b,$dd,$42,$fe,$02,$5d,$c7
    .byte $fd
; level 3-3
L_GroundArea1:
    .byte $94,$11
    .byte $0f,$26,$fe,$10,$28,$94,$65,$15,$eb,$12,$fa,$41,$4a,$96,$54,$40
    .byte $a4,$42,$b7,$13,$e9,$19,$f5,$15,$11,$80,$47,$42,$71,$13,$80,$41
    .byte $15,$92,$1b,$1f,$24,$40,$55,$12,$64,$40,$95,$12,$a4,$40,$d2,$12
    .byte $e1,$40,$13,$c0,$2c,$17,$2f,$12,$49,$13,$83,$40,$9f,$14,$a3,$40
    .byte $17,$92,$83,$13,$92,$41,$b9,$14,$c5,$12,$c8,$40,$d4,$40,$4b,$92
    .byte $78,$1b,$9c,$94,$9f,$11,$df,$14,$fe,$11,$7d,$c1,$9e,$42,$cf,$20
    .byte $fd
; level 8-3
L_GroundArea2:
    .byte $90,$b1
    .byte $0f,$26,$29,$91,$7e,$42,$fe,$40,$28,$92,$4e,$42,$2e,$c0,$57,$73
    .byte $c3,$25,$c7,$27,$23,$84,$33,$20,$5c,$01,$77,$63,$88,$62,$99,$61
    .byte $aa,$60,$bc,$01,$ee,$42,$4e,$c0,$69,$11,$7e,$42,$de,$40,$f8,$62
    .byte $0e,$c2,$ae,$40,$d7,$63,$e7,$63,$33,$a7,$37,$27,$43,$04,$cc,$01
    .byte $e7,$73,$0c,$81,$3e,$42,$0d,$0a,$5e,$40,$88,$72,$be,$42,$e7,$87
    .byte $fe,$40,$39,$e1,$4e,$00,$69,$60,$87,$60,$a5,$60,$c3,$31,$fe,$31
    .byte $6d,$c1,$be,$42,$ef,$20
    .byte $fd
; level 4-1
L_GroundArea3:
    .byte $52,$21
    .byte $0f,$20,$6e,$40,$58,$f2,$93,$01,$97,$00,$0c,$81,$97,$40,$a6,$41
    .byte $c7,$40,$0d,$04,$03,$01,$07,$01,$23,$01,$27,$01,$ec,$03,$ac,$f3
    .byte $c3,$03,$78,$e2,$94,$43,$47,$f3,$74,$43,$47,$fb,$74,$43,$2c,$f1
    .byte $4c,$63,$47,$00,$57,$21,$5c,$01,$7c,$72,$39,$f1,$ec,$02,$4c,$81
    .byte $d8,$62,$ec,$01,$0d,$0d,$0f,$38,$c7,$07,$ed,$4a,$1d,$c1,$5f,$26
    .byte $fd
; level 6-2
L_GroundArea4:
    .byte $54,$21
    .byte $0f,$26,$a7,$22,$37,$fb,$73,$20,$83,$07,$87,$02,$93,$20,$c7,$73
    .byte $04,$f1,$06,$31,$39,$71,$59,$71,$e7,$73,$37,$a0,$47,$04,$86,$7c
    .byte $e5,$71,$e7,$31,$33,$a4,$39,$71,$a9,$71,$d3,$23,$08,$f2,$13,$05
    .byte $27,$02,$49,$71,$75,$75,$e8,$72,$67,$f3,$99,$71,$e7,$20,$f4,$72
    .byte $f7,$31,$17,$a0,$33,$20,$39,$71,$73,$28,$bc,$05,$39,$f1,$79,$71
    .byte $a6,$21,$c3,$06,$d3,$20,$dc,$00,$fc,$00,$07,$a2,$13,$21,$5f,$32
    .byte $8c,$00,$98,$7a,$c7,$63,$d9,$61,$03,$a2,$07,$22,$74,$72,$77,$31
    .byte $e7,$73,$39,$f1,$58,$72,$77,$73,$d8,$72,$7f,$b1,$97,$73,$b6,$64
    .byte $c5,$65,$d4,$66,$e3,$67,$f3,$67,$8d,$c1,$cf,$26
    .byte $fd
; level 3-1
L_GroundArea5:
    .byte $52,$31
    .byte $0f,$20,$6e,$66,$07,$81,$36,$01,$66,$00,$a7,$22,$08,$f2,$67,$7b
    .byte $dc,$02,$98,$f2,$d7,$20,$39,$f1,$9f,$33,$dc,$27,$dc,$57,$23,$83
    .byte $57,$63,$6c,$51,$87,$63,$99,$61,$a3,$06,$b3,$21,$77,$f3,$f3,$21
    .byte $f7,$2a,$13,$81,$23,$22,$53,$00,$63,$22,$e9,$0b,$0c,$83,$13,$21
    .byte $16,$22,$33,$05,$8f,$35,$ec,$01,$63,$a0,$67,$20,$73,$01,$77,$01
    .byte $83,$20,$87,$20,$b3,$20,$b7,$20,$c3,$01,$c7,$00,$d3,$20,$d7,$20
    .byte $67,$a0,$77,$07,$87,$22,$e8,$62,$f5,$65,$1c,$82,$7f,$38,$8d,$c1
    .byte $cf,$26
    .byte $fd
; level 1-1
L_GroundArea6:
    .byte $50,$21
    .byte $07,$81,$47,$24,$57,$00,$63,$01,$77,$01,$c9,$71,$68,$f2,$e7,$73
    .byte $97,$fb,$06,$83,$5c,$01,$d7,$22,$e7,$00,$03,$a7,$6c,$02,$b3,$22
    .byte $e3,$01,$e7,$07,$47,$a0,$57,$06,$a7,$01,$d3,$00,$d7,$01,$07,$81
...
L_GroundArea7:
    .byte $90,$11
    .byte $0f,$26,$fe,$10,$2a,$93,$87,$17,$a3,$14,$b2,$42,$0a,$92,$19,$40
    .byte $36,$14,$50,$41,$82,$16,$2b,$93,$24,$41,$bb,$14,$b8,$00,$c2,$43
    .byte $c3,$13,$1b,$94,$67,$12,$c4,$15,$53,$c1,$d2,$41,$12,$c1,$29,$13
...
L_GroundArea8:
    .byte $90,$11
    .byte $0f,$26,$6e,$10,$8b,$17,$af,$32,$d8,$62,$e8,$62,$fc,$3f,$ad,$c8
    .byte $f8,$64,$0c,$be,$43,$43,$f8,$64,$0c,$bf,$73,$40,$84,$40,$93,$40
    .byte $a4,$40,$b3,$40,$f8,$64,$48,$e4,$5c,$39,$83,$40,$92,$41,$b3,$40
    .byte $f8,$64,$48,$e4,$5c,$39,$f8,$64,$13,$c2,$37,$65,$4c,$24,$63,$00
    .byte $97,$65,$c3,$42,$0b,$97,$ac,$32,$f8,$64,$0c,$be,$53,$45,$9d,$48
    .byte $f8,$64,$2a,$e2,$3c,$47,$56,$43,$ba,$62,$f8,$64,$0c,$b7,$88,$64
    .byte $bc,$31,$d4,$45,$fc,$31,$3c,$b1,$78,$64,$8c,$38,$0b,$9c,$1a,$33
    .byte $18,$61,$28,$61,$39,$60,$5d,$4a,$ee,$11,$0f,$b8,$1d,$c1,$3e,$42
    .byte $6f,$20
    .byte $fd
; level 2-1
L_GroundArea9:
    .byte $52,$31
    .byte $0f,$20,$6e,$40,$f7,$20,$07,$84,$17,$20,$4f,$34,$c3,$03,$c7,$02
    .byte $d3,$22,$27,$e3,$39,$61,$e7,$73,$5c,$e4,$57,$00,$6c,$73,$47,$a0
    .byte $53,$06,$63,$22,$a7,$73,$fc,$73,$13,$a1,$33,$05,$43,$21,$5c,$72
    .byte $c3,$23,$cc,$03,$77,$fb,$ac,$02,$39,$f1,$a7,$73,$d3,$04,$e8,$72
    .byte $e3,$22,$26,$f4,$bc,$02,$8c,$81,$a8,$62,$17,$87,$43,$24,$a7,$01
    .byte $c3,$04,$08,$f2,$97,$21,$a3,$02,$c9,$0b,$e1,$69,$f1,$69,$8d,$c1
    .byte $cf,$26
    .byte $fd
; pipe intro area
L_GroundArea10:
...
L_GroundArea11:
    .byte $95,$b1
    .byte $0f,$26,$0d,$02,$c8,$72,$1c,$81,$38,$72,$0d,$05,$97,$34,$98,$62
    .byte $a3,$20,$b3,$06,$c3,$20,$cc,$03,$f9,$91,$2c,$81,$48,$62,$0d,$09
    .byte $37,$63,$47,$03,$57,$21,$8c,$02,$c5,$79,$c7,$31,$f9,$11,$39,$f1
    .byte $a9,$11,$6f,$b4,$d3,$65,$e3,$65,$7d,$c1,$bf,$26
    .byte $fd
; cloud level used in levels 2-1 and 5-2
L_GroundArea12:
    .byte $00,$c1
    .byte $4c,$00,$f4,$4f,$0d,$02,$02,$42,$43,$4f,$52,$c2,$de,$00,$5a,$c2
    .byte $4d,$c7
    .byte $fd
; level 4-3
L_GroundArea13:
    .byte $90,$51
    .byte $0f,$26,$ee,$10,$0b,$94,$33,$14,$42,$42,$77,$16,$86,$44,$02,$92
    .byte $4a,$16,$69,$42,$73,$14,$b0,$00,$c7,$12,$05,$c0,$1c,$17,$1f,$11
    .byte $36,$12,$8f,$14,$91,$40,$1b,$94,$35,$12,$34,$42,$60,$42,$61,$12
    .byte $87,$12,$96,$40,$a3,$14,$1c,$98,$1f,$11,$47,$12,$9f,$15,$cc,$15
    .byte $cf,$11,$05,$c0,$1f,$15,$39,$12,$7c,$16,$7f,$11,$82,$40,$98,$12
    .byte $df,$15,$16,$c4,$17,$14,$54,$12,$9b,$16,$28,$94,$ce,$01,$3d,$c1
    .byte $5e,$42,$8f,$20
    .byte $fd
; level 6-3
L_GroundArea14:
    .byte $97,$11
    .byte $0f,$26,$fe,$10,$2b,$92,$57,$12,$8b,$12,$c0,$41,$f7,$13,$5b,$92
    .byte $69,$0b,$bb,$12,$b2,$46,$19,$93,$71,$00,$17,$94,$7c,$14,$7f,$11
    .byte $93,$41,$bf,$15,$fc,$13,$ff,$11,$2f,$95,$50,$42,$51,$12,$58,$14
    .byte $a6,$12,$db,$12,$1b,$93,$46,$43,$7b,$12,$8d,$49,$b7,$14,$1b,$94
    .byte $49,$0b,$bb,$12,$fc,$13,$ff,$12,$03,$c1,$2f,$15,$43,$12,$4b,$13
    .byte $77,$13,$9d,$4a,$15,$c1,$a1,$41,$c3,$12,$fe,$01,$7d,$c1,$9e,$42
    .byte $cf,$20
    .byte $fd
; level 6-1
L_GroundArea15:
    .byte $52,$21
    .byte $0f,$20,$6e,$44,$0c,$f1,$4c,$01,$aa,$35,$d9,$34,$ee,$20,$08,$b3
    .byte $37,$32,$43,$04,$4e,$21,$53,$20,$7c,$01,$97,$21,$b7,$07,$9c,$81
    .byte $e7,$42,$5f,$b3,$97,$63,$ac,$02,$c5,$41,$49,$e0,$58,$61,$76,$64
    .byte $85,$65,$94,$66,$a4,$22,$a6,$03,$c8,$22,$dc,$02,$68,$f2,$96,$42
    .byte $13,$82,$17,$02,$af,$34,$f6,$21,$fc,$06,$26,$80,$2a,$24,$36,$01
    .byte $8c,$00,$ff,$35,$4e,$a0,$55,$21,$77,$20,$87,$07,$89,$22,$ae,$21
    .byte $4c,$82,$9f,$34,$ec,$01,$03,$e7,$13,$67,$8d,$4a,$ad,$41,$0f,$a6
    .byte $fd
; warp zone area used in level 4-2
L_GroundArea16:
    .byte $10,$51
    .byte $4c,$00,$c7,$12,$c6,$42,$03,$92,$02,$42,$29,$12,$63,$12,$62,$42
    .byte $69,$14,$a5,$12,$a4,$42,$e2,$14,$e1,$44,$f8,$16,$37,$c1,$8f,$38
    .byte $02,$bb,$28,$7a,$68,$7a,$a8,$7a,$e0,$6a,$f0,$6a,$6d,$c5
    .byte $fd
; level 8-1
L_GroundArea17:
    .byte $92,$31
    .byte $0f,$20,$6e,$40,$0d,$02,$37,$73,$ec,$00,$0c,$80,$3c,$00,$6c,$00
    .byte $9c,$00,$06,$c0,$c7,$73,$06,$83,$28,$72,$96,$40,$e7,$73,$26,$c0
    .byte $87,$7b,$d2,$41,$39,$f1,$c8,$f2,$97,$e3,$a3,$23,$e7,$02,$e3,$07
    .byte $f3,$22,$37,$e3,$9c,$00,$bc,$00,$ec,$00,$0c,$80,$3c,$00,$86,$21
    .byte $a6,$06,$b6,$24,$5c,$80,$7c,$00,$9c,$00,$29,$e1,$dc,$05,$f6,$41
    .byte $dc,$80,$e8,$72,$0c,$81,$27,$73,$4c,$01,$66,$74,$0d,$11,$3f,$35
    .byte $b6,$41,$2c,$82,$36,$40,$7c,$02,$86,$40,$f9,$61,$39,$e1,$ac,$04
    .byte $c6,$41,$0c,$83,$16,$41,$88,$f2,$39,$f1,$7c,$00,$89,$61,$9c,$00
    .byte $a7,$63,$bc,$00,$c5,$65,$dc,$00,$e3,$67,$f3,$67,$8d,$c1,$cf,$26
    .byte $fd
; level 5-2
L_GroundArea18:
    .byte $55,$b1
    .byte $0f,$26,$cf,$33,$07,$b2,$15,$11,$52,$42,$99,$0b,$ac,$02,$d3,$24
    .byte $d6,$42,$d7,$25,$23,$84,$cf,$33,$07,$e3,$19,$61,$78,$7a,$ef,$33
    .byte $2c,$81,$46,$64,$55,$65,$65,$65,$ec,$74,$47,$82,$53,$05,$63,$21
    .byte $62,$41,$96,$22,$9a,$41,$cc,$03,$b9,$91,$39,$f1,$63,$26,$67,$27
    .byte $d3,$06,$fc,$01,$18,$e2,$d9,$07,$e9,$04,$0c,$86,$37,$22,$93,$24
    .byte $87,$84,$ac,$02,$c2,$41,$c3,$23,$d9,$71,$fc,$01,$7f,$b1,$9c,$00
    .byte $a7,$63,$b6,$64,$cc,$00,$d4,$66,$e3,$67,$f3,$67,$8d,$c1,$cf,$26
    .byte $fd
; level 8-2
L_GroundArea19:
    .byte $50,$b1
    .byte $0f,$26,$fc,$00,$1f,$b3,$5c,$00,$65,$65,$74,$66,$83,$67,$93,$67
    .byte $dc,$73,$4c,$80,$b3,$20,$c9,$0b,$c3,$08,$d3,$2f,$dc,$00,$2c,$80
    .byte $4c,$00,$8c,$00,$d3,$2e,$ed,$4a,$fc,$00,$d7,$a1,$ec,$01,$4c,$80
    .byte $59,$11,$d8,$11,$da,$10,$37,$a0,$47,$04,$99,$11,$e7,$21,$3a,$90
    .byte $67,$20,$76,$10,$77,$60,$87,$07,$d8,$12,$39,$f1,$ac,$00,$e9,$71
    .byte $0c,$80,$2c,$00,$4c,$05,$c7,$7b,$39,$f1,$ec,$00,$f9,$11,$0c,$82
    .byte $6f,$34,$f8,$11,$fa,$10,$7f,$b2,$ac,$00,$b6,$64,$cc,$01,$e3,$67
    .byte $f3,$67,$8d,$c1,$cf,$26
    .byte $fd
; level 7-1
L_GroundArea20:
    .byte $52,$b1
    .byte $0f,$20,$6e,$45,$39,$91,$b3,$04,$c3,$21,$c8,$11,$ca,$10,$49,$91
    .byte $7c,$73,$e8,$12,$88,$91,$8a,$10,$e7,$21,$05,$91,$07,$30,$17,$07
    .byte $27,$20,$49,$11,$9c,$01,$c8,$72,$23,$a6,$27,$26,$d3,$03,$d8,$7a
    .byte $89,$91,$d8,$72,$39,$f1,$a9,$11,$09,$f1,$63,$24,$67,$24,$d8,$62
    .byte $28,$91,$2a,$10,$56,$21,$70,$04,$79,$0b,$8c,$00,$94,$21,$9f,$35
    .byte $2f,$b8,$3d,$c1,$7f,$26
    .byte $fd
; cloud level used in levels 3-1 and 6-2
L_GroundArea21:
    .byte $06,$c1
    .byte $4c,$00,$f4,$4f,$0d,$02,$06,$20,$24,$4f,$35,$a0,$36,$20,$53,$46
    .byte $d5,$20,$d6,$20,$34,$a1,$73,$49,$74,$20,$94,$20,$b4,$20,$d4,$20
    .byte $f4,$20,$2e,$80,$59,$42,$4d,$c7
    .byte $fd
; level 3-2
L_GroundArea22:
    .byte $96,$31
    .byte $0f,$26,$0d,$03,$1a,$60,$77,$42,$c4,$00,$c8,$62,$b9,$e1,$d3,$06
    .byte $d7,$07,$f9,$61,$0c,$81,$4e,$b1,$8e,$b1,$bc,$01,$e4,$50,$e9,$61
    .byte $0c,$81,$0d,$0a,$84,$43,$98,$72,$0d,$0c,$0f,$38,$1d,$c1,$5f,$26
    .byte $fd
; level 1-2
L_UndergroundArea1:
    .byte $48,$0f
    .byte $0e,$01,$5e,$02,$a7,$00,$bc,$73,$1a,$e0,$39,$61,$58,$62,$77,$63
    .byte $97,$63,$b8,$62,$d6,$07,$f8,$62,$19,$e1,$75,$52,$86,$40,$87,$50
    .byte $95,$52,$93,$43,$a5,$21,$c5,$52,$d6,$40,$d7,$20,$e5,$06,$e6,$51
    .byte $3e,$8d,$5e,$03,$67,$52,$77,$52,$7e,$02,$9e,$03,$a6,$43,$a7,$23
    .byte $de,$05,$fe,$02,$1e,$83,$33,$54,$46,$40,$47,$21,$56,$04,$5e,$02
    .byte $83,$54,$93,$52,$96,$07,$97,$50,$be,$03,$c7,$23,$fe,$02,$0c,$82
    .byte $43,$45,$45,$24,$46,$24,$90,$08,$95,$51,$78,$fa,$d7,$73,$39,$f1
    .byte $8c,$01,$a8,$52,$b8,$52,$cc,$01,$5f,$b3,$97,$63,$9e,$00,$0e,$81
    .byte $16,$24,$66,$04,$8e,$00,$fe,$01,$08,$d2,$0e,$06,$6f,$47,$9e,$0f
    .byte $0e,$82,$2d,$47,$28,$7a,$68,$7a,$a8,$7a,$ae,$01,$de,$0f,$6d,$c5
    .byte $fd
; level 4-2
L_UndergroundArea2:
    .byte $48,$0f
    .byte $0e,$01,$5e,$02,$bc,$01,$fc,$01,$2c,$82,$41,$52,$4e,$04,$67,$25
    .byte $68,$24,$69,$24,$ba,$42,$c7,$04,$de,$0b,$b2,$87,$fe,$02,$2c,$e1
    .byte $2c,$71,$67,$01,$77,$00,$87,$01,$8e,$00,$ee,$01,$f6,$02,$03,$85
    .byte $05,$02,$13,$21,$16,$02,$27,$02,$2e,$02,$88,$72,$c7,$20,$d7,$07
    .byte $e4,$76,$07,$a0,$17,$06,$48,$7a,$76,$20,$98,$72,$79,$e1,$88,$62
    .byte $9c,$01,$b7,$73,$dc,$01,$f8,$62,$fe,$01,$08,$e2,$0e,$00,$6e,$02
    .byte $73,$20,$77,$23,$83,$04,$93,$20,$ae,$00,$fe,$0a,$0e,$82,$39,$71
    .byte $a8,$72,$e7,$73,$0c,$81,$8f,$32,$ae,$00,$fe,$04,$04,$d1,$17,$04
    .byte $26,$49,$27,$29,$df,$33,$fe,$02,$44,$f6,$7c,$01,$8e,$06,$bf,$47
    .byte $ee,$0f,$4d,$c7,$0e,$82,$68,$7a,$ae,$01,$de,$0f,$6d,$c5
    .byte $fd
; underground bonus rooms area used in many levels
L_UndergroundArea3:
    .byte $48,$01
    .byte $0e,$01,$00,$5a,$3e,$06,$45,$46,$47,$46,$53,$44,$ae,$01,$df,$4a
    .byte $4d,$c7,$0e,$81,$00,$5a,$2e,$04,$37,$28,$3a,$48,$46,$47,$c7,$07
    .byte $ce,$0f,$df,$4a,$4d,$c7,$0e,$81,$00,$5a,$33,$53,$43,$51,$46,$40
    .byte $47,$50,$53,$04,$55,$40,$56,$50,$62,$43,$64,$40,$65,$50,$71,$41
    .byte $73,$51,$83,$51,$94,$40,$95,$50,$a3,$50,$a5,$40,$a6,$50,$b3,$51
    .byte $b6,$40,$b7,$50,$c3,$53,$df,$4a,$4d,$c7,$0e,$81,$00,$5a,$2e,$02
    .byte $36,$47,$37,$52,$3a,$49,$47,$25,$a7,$52,$d7,$04,$df,$4a,$4d,$c7
    .byte $0e,$81,$00,$5a,$3e,$02,$44,$51,$53,$44,$54,$44,$55,$24,$a1,$54
    .byte $ae,$01,$b4,$21,$df,$4a,$e5,$07,$4d,$c7
    .byte $fd
; water area used in levels 5-2 and 6-2
L_WaterArea1:
    .byte $41,$01
    .byte $b4,$34,$c8,$52,$f2,$51,$47,$d3,$6c,$03,$65,$49,$9e,$07,$be,$01
    .byte $cc,$03,$fe,$07,$0d,$c9,$1e,$01,$6c,$01,$62,$35,$63,$53,$8a,$41
    .byte $ac,$01,$b3,$53,$e9,$51,$26,$c3,$27,$33,$63,$43,$64,$33,$ba,$60
    .byte $c9,$61,$ce,$0b,$e5,$09,$ee,$0f,$7d,$ca,$7d,$47
    .byte $fd
; level 2-2/7-2
L_WaterArea2:
    .byte $41,$01
    .byte $b8,$52,$ea,$41,$27,$b2,$b3,$42,$16,$d4,$4a,$42,$a5,$51,$a7,$31
    .byte $27,$d3,$08,$e2,$16,$64,$2c,$04,$38,$42,$76,$64,$88,$62,$de,$07
    .byte $fe,$01,$0d,$c9,$23,$32,$31,$51,$98,$52,$0d,$c9,$59,$42,$63,$53
    .byte $67,$31,$14,$c2,$36,$31,$87,$53,$17,$e3,$29,$61,$30,$62,$3c,$08
    .byte $42,$37,$59,$40,$6a,$42,$99,$40,$c9,$61,$d7,$63,$39,$d1,$58,$52
    .byte $c3,$67,$d3,$31,$dc,$06,$f7,$42,$fa,$42,$23,$b1,$43,$67,$c3,$34
    .byte $c7,$34,$d1,$51,$43,$b3,$47,$33,$9a,$30,$a9,$61,$b8,$62,$be,$0b
    .byte $d5,$09,$de,$0f,$0d,$ca,$7d,$47
    .byte $fd
; water area used in level 8-4
L_WaterArea3:
    .byte $49,$0f
    .byte $1e,$01,$39,$73,$5e,$07,$ae,$0b,$1e,$82,$6e,$88,$9e,$02,$0d,$04
    .byte $2e,$0b,$45,$09,$4e,$0f,$ed,$47
    .byte $fd
; -----------------------------------------------------------------------------
; unused space
    .byte $ff
...
GameCoreRoutine:
    ...
    lda SavedJoypad1Bits,x ;use appropriate player's controller bits
    sta SavedJoypad1Bits ; as the master controller bits
...
KeepOnscr:
    lda ScreenLeft_X_Pos,y ;get left or right side coordinate based on offset
    sec
    ...
    lda ScreenLeft_PageLoc,y ;get left or right page location based on offset
    sbc #$00 ;subtract borrow
...
AutoControlPlayer:
    sta SavedJoypad1Bits ;override controller bits with contents of A if executing here
PlayerCtrlRoutine:
    lda GameEngineSubroutine ;check task here
    cmp #$0b ;if certain value is set, branch to skip controller bit 
    beq SizeChk
...
DisJoyp:
    ...
    sta SavedJoypad1Bits
SaveJoyp:
    lda SavedJoypad1Bits ;otherwise store A and B buttons in $0a
    and #%11000000
    sta A_B_Buttons
    lda SavedJoypad1Bits ;store left and right buttons in $0c
    and #%00000011
    sta Left_Right_Buttons
    lda SavedJoypad1Bits ;store up and down buttons in $0b
    and #%00001100
    sta Up_Down_Buttons
...
ProcMove:
    ...
    ldy #$18
    sty ClimbSlideTimer ;otherwise reset timer now
MoveSubs:
    jsr JumpEngine
...
MoveOnVine:
    ...
    ldy ClimbSlideTimer ;otherwise check timer 
    bne ExitCSub ;if timer not expired, branch to leave
    ldy #$18
    sty ClimbSlideTimer ;otherwise set timer now
...
InitCSTimer:
    sta ClimbSlideTimer ;initialize timer here
    rts
...
ChkSkid:
    lda SavedJoypad1Bits ;get controller bits
...
SetupBB:
    ...
    sta BlooperMoveSpeed,x ;and store it
...
; -----------------------------------------------------------------------------
HammerEnemyOfsData:
    .byte $04,$04,$04,$05,$05,$05,$06,$06,$06
HammerXSpdData:
    .byte $10,$f0

SpawnHammerObj:
    lda PseudoRandomBitReg+1 ;get pseudorandom bits from
    and #$07 ; second part of LSFR
...
UseAdder:
    ...
    sta Player_X_Position,x ; plus $f0 if necessary) to object's horizontal position
    lda Player_PageLoc,x
    adc $02 ;add carry plus other saved value to the
    sta Player_PageLoc,x ; object's page location and save
    pla
...
MovePlatformUp:
    ...
    bne SetDblSpd ; this code, thus unconditional branch here
...
SetESpd:
    sta BlooperMoveSpeed,x ;store as speed for enemy object
...
SpinyRte:
    ...
    lda #$fd
    sta BlooperMoveCounter,x ;set vertical speed to move upwards
    lda #$01
...
MovePodoboo:
    ...
    lda #$f9
    sta BlooperMoveCounter,x ;set vertical speed to move podoboo upwards
PdbM:
...
ChkKillGoomba:
    ...
    bne HKGmba ; a certain point, and branch to leave if not
    lda Enemy_ID,x
    cmp #Goomba ;check for goomba object
    bne HKGmba ;branch if not found
...
ProcSwimmingB:
    ...
    pha ;get 3 LSB of frame counter
    lda BlooperMoveCounter,x ;get enemy's movement counter
    lsr ;check for d0 set
...
ChkForFloatdown:
    ...
    beq ChkNeearPlayer ;branch if expired
Floatdown:
    lda FrameCounter ;get frame counter
    lsr ;check for d0 set
...
FirebarPosLookupTbl:
    .byte $00,$01,$03,$04,$05,$06,$07,$07,$08,$00,$03,$06,$09,$0b,$0d,$0e
    .byte $0f,$10,$00,$04,$09,$0d,$10,$13,$16,$17,$18,$00,$06,$0c,$12,$16
    .byte $1a,$1d,$1f,$20,$00,$07,$0f,$16,$1c,$21,$25,$27,$28,$00,$09,$12
    .byte $1b,$21,$27,$2c,$2f,$30,$00,$0b,$15,$1f,$27,$2e,$33,$37,$38,$00
    .byte $0c,$18,$24,$2d,$35,$3b,$3e,$40,$00,$0e,$1b,$28,$32,$3b,$42,$46
    .byte $48,$00,$0f,$1f,$2d,$38,$42,$4a,$4e,$50,$00,$11,$22,$31,$3e,$49
    .byte $51,$56,$58
FirebarMirrorData:
    .byte $01,$03,$02,$00
FirebarTblOffsets:
...
ChkSpinyO:
    ...
    bne SubDifAdj
ChkEmySpd:
    lda LakituMoveDirection,x ;check vertical speed
...
SetFBTmr:
    ...
    lda #$15 ;put bowser's flame identifier
...
DrawFlameLoop:
    ...
    sta Sprite_Y_Position+$0c,y ; residual since flame is only made of three sprites
M3FOfs:
...
ChkYPCollision:
    lda HammerThrowingTimer,x ;if collision flag not set here, branch
    bmi ExYPl ; to leave
...
LimitB:
    adc #56 ;add 56 pixels to coordinate if hammer bro or piranha plant
ExtendLB:
    sbc #72 ;subtract 72 pixels regardless of enemy object
    sta $01 ;store result here
    lda ScreenLeft_PageLoc
...
GoombaDie:
    ...
    bne NoGoomba ;if not found, continue with code
...
SetDBSte:
    ...
    bne EnemySmackCore ;unconditional branch to award points
...
GoombaPoints:
    ...
    bne EnemySmackCore ;branch if not found
...
PlayerEnemyCollision:
    ...
    ldx ObjectOffset ;get enemy object buffer offset
    bcs ChkForPUpCollision ;if collision, branch past this part here
...
HeadChk:
    ...
    beq MYSpd ;if water level, branch ahead
    ldy BlockBounceTimer ;if block bounce timer not expired,
    bne MYSpd ; branch ahead, do not process collision
...
SolidOrClimb:
    ...
    beq MYSpd ; branch ahead and do not play sound
    lda #Sfx_Bump
...
SideCheckLoop:
    ...
    bcc ChkSideMTiles ;if not, branch to alternate section of code
BHalf:
    ...
    bne ChkSideMTiles ;if something found, branch
...
BoundingBoxCore:
    ...
    sta BoundingBox_UL_XPos,y ;store here
    lda $01
    clc
    adc BoundBoxCtrlData+2,x ;add the third number in the bounding box data to the
    sta BoundingBox_DR_XPos,y ; relative horizontal coordinate and store
    ...
    sta BoundingBox_DR_XPos,y ;and store
...
CollisionCoreLoop:
    lda BoundingBox_UL_XPos,y ;compare left/top coordinates
    cmp BoundingBox_UL_XPos,x ; of first and second objects' bounding boxes
    bcs FirstBoxGreater ;if first left/top => second, branch
    cmp BoundingBox_DR_XPos,x ;otherwise compare to right/bottom of second
    ...
    lda BoundingBox_DR_XPos,y ;if somehow greater, check to see if bottom of
    cmp BoundingBox_UL_XPos,y ; first object's bounding box is greater than its top
    bcc CollisionFound ;if somehow less, vertical wrap collision, thus branch
    cmp BoundingBox_UL_XPos,x ;otherwise compare bottom of first bounding box to the top
...
SecondBoxVerticalChk:
    lda BoundingBox_DR_XPos,x ;check to see if the vertical bottom of the box
    cmp BoundingBox_UL_XPos,x ; is greater than the vertical top
    bcc CollisionFound ;if somehow less, vertical wrap collision, thus branch
    lda BoundingBox_DR_XPos,y ;otherwise compare horizontal right or vertical bottom
    cmp BoundingBox_UL_XPos,x ; of first box with horizontal left or vertical top of second box
    bcs CollisionFound ;if equal or greater, collision, thus branch
    ldy $06 ;otherwise return with carry clear and Y = $0006
    rts

FirstBoxGreater:
    cmp BoundingBox_UL_XPos,x ;compare first and second box horizontal left/vertical top again
    beq CollisionFound ;if first coordinate = second, collision, thus branch
    cmp BoundingBox_DR_XPos,x ;if not, compare with second object right or bottom edge
    ...
    cmp BoundingBox_DR_XPos,y ;otherwise check to see if top of first box is greater than bottom
    bcc NoCollisionFound ;if less than or equal, no collision, branch to end
    beq NoCollisionFound
    lda BoundingBox_DR_XPos,y ;otherwise compare bottom of first to top of second
    cmp BoundingBox_UL_XPos,x ;if bottom of first is greater than top of second, vertical wrap
...
BlockBufferAdderData:
    .byte $00,$07,$0e
BlockBuffer_X_Adder:
    .byte $08,$03,$0c,$02,$02,$0d,$0d,$08,$03,$0c,$02,$02,$0d,$0d,$08,$03
    .byte $0c,$02,$02,$0d,$0d,$08,$00,$10,$04,$14,$04,$04
BlockBuffer_Y_Adder:
    .byte $04,$20,$20,$08,$18,$08,$18,$02,$20,$20,$08,$18,$08,$18,$12,$20
    .byte $20,$18,$18,$18,$18,$18,$14,$14,$06,$06,$08,$10
...
BlockBufferCollision:
    ...
    sec
    sbc #$20 ;subtract 32 pixels for the status ba
    sta $02 ;store result here
...
ForceHPose:
    ...
    beq RdnerH ;do unconditional branch to rendering part
...
DumpSixSpr:
    sta Sprite_Data+$14,y ;dump A contents
    sta Sprite_Data+$10,y ;into third row sprites
DumpFourSpr:
    sta Sprite_Data+$0c,y ;into second row sprites
DumpThreeSpr:
    sta Sprite_Data+8,y
DumpTwoSpr:
...
SetLast2Platform:
    ldy Enemy_SprDataOffset,x ;get OAM data offset
    sta Sprite_Y_Position+$10,y ;store vertical coordinate or offscreen
    sta Sprite_Y_Position+$14,y ; coordinate into last two sprites as Y coordinate
...
SChk5:
    ...
    sta Sprite_Y_Position+$10,y
SChk6:
    pla ;get bits from stack
    asl ;rotate d2 into carry
    bcc SLChk ;save to stack
    lda #$f8
    sta Sprite_Y_Position+$14,y ;if d2 was set, move sixth sprite offscreen
SLChk:
...
PUpDrawLoop:
    ...
    sta Sprite_Attributes+$0c,y ; and bottom right sprites as well for star only
FlipPUpRightSide:
    lda Sprite_Attributes+4,y
    ora #%01000000 ;set horizontal flip bit for top right sprite
    sta Sprite_Attributes+4,y
    lda Sprite_Attributes+$0c,y
    ora #%01000000 ;set horizontal flip bit for bottom right sprite
    sta Sprite_Attributes+$0c,y ;note these are only done for fire flower and star power-ups
PUpOfs:
...
EnemyGfxTableOffsets:
    .byte $0c,$0c,$00,$0c,$0c,$a8,$54,$3c,$ea,$18,$48,$48,$cc,$c0,$18,$18
    .byte $18,$90,$24,$ff,$48,$9c,$d2,$d8,$f0,$f6,$fc
EnemyAttributeData:
    .byte $01,$02,$03,$02,$01,$01,$03,$03,$03,$01,$01,$02,$02,$21,$01,$02
    .byte $01,$01,$02,$ff,$02,$02,$01,$01,$02,$02,$02
...
CheckBowserFront:
    ...
    bpl CheckFrontSte ;branch if d7 not set (control's bowser's mouth) 
...
FlipEnemyVertically:
    ...
    pha
    lda Sprite_Tilenumber+$10,y ;exchange third row tiles
    sta Sprite_Tilenumber,x ; with first or second row tiles
    lda Sprite_Tilenumber+$14,y
    sta Sprite_Tilenumber+4,x
    pla ;pull first or second row tiles from stack
    sta Sprite_Tilenumber+$14,y ; and save in third row
    pla
    sta Sprite_Tilenumber+$10,y
CheckForESymmetry:
...
ESRtnr:
    ...
    sta Sprite_Attributes+$14,y ;note that palette bits were already set earlier
SpnySC:
...
EggExc:
    ...
    sta Sprite_Attributes+$0c,y ;of enemy object sprite data
    sta Sprite_Attributes+$14,y
    ...
    sta Sprite_Attributes+$10,y ; second and third row left sprites
    ora #$40
    sta Sprite_Attributes+$0c,y ;store with horizontal and vertical flip in
    sta Sprite_Attributes+$14,y ; second and third row right sprites
CheckToMirrorLakitu:
    ...
    lda Sprite_Attributes+$10,y ;save vertical flip and palette bits
    and #%10000001 ; in third row left sprite
    sta Sprite_Attributes+$10,y
    lda Sprite_Attributes+$14,y ;set horizontal flip and palette bits
    ...
    sta Sprite_Attributes+$0c,y ;otherwise set same for second row right sprite
    and #%10000001
...
CheckToMirrorJSpring:
    ...
    sta Sprite_Attributes+$10,y ; second and third row left sprites
    ora #$40
    sta Sprite_Attributes+$0c,y ;set, in addition to those, horizontal flip
    sta Sprite_Attributes+$14,y ; for second and third row right sprites
SprObjectOffscrChk:
...
MoveESprColOffscreen:
    ...
    sta Sprite_Data+$10,y ;move third row sprite in column offscreen
    rts
...
SetBFlip:
    ...
    ora #$80
    sta Sprite_Attributes+$0c,y ;set both flip bits for fourth sprite
    and #$83
...
DChunks:
    ...
    sta Sprite_Y_Position+8,y
    sta Sprite_Y_Position+$0c,y ;dump into Y coordinates of third and fourth sprites
    ...
    sta Sprite_X_Position+$0c,y ;save into X coordinate of fourth sprite
    lda Block_OffscreenBits ;get offscreen bits for block object
...
DrawExplosion_Fireworks:
    ...
    lda #$c2
    sta Sprite_Attributes+$0c,y ;set both flips for fourth sprite
    rts ;we are done
...
DrawSmallPlatform:
    ...
    lda Enemy_Rel_XPos ;get relative horizontal coordinate
    sta Sprite_X_Position,y
    sta Sprite_X_Position+$0c,y ;dump as X coordinate into first and fourth sprites
    clc
    adc #$08 ;add eight pixels
...
BotSP:
    sta Sprite_Y_Position+$0c,y ;dump vertical coordinate + 128 pixels
    sta Sprite_Y_Position+$10,y ; into Y coordinates
    sta Sprite_Y_Position+$14,y
    lda Enemy_OffscreenBits ;get offscreen bits
    pha ;save to stack
...
PlayerGfxTblOffsets:
    .byte $20,$28,$c8,$18,$00,$40,$50,$58,$80,$88,$b8,$78,$60,$a0,$b0,$b8
; 
...
CntPl:
    ...
    ldy SprDataOffset ;get player sprite data offset
    lda PlayerFacingDir ;get player's facing direction
    lsr
...
SwimKT:
    ...
    lda Sprite_Tilenumber+$18,y ;check tile number of seventh/eighth sprite
    cmp SwimTiles+$16 ; against tile number in player graphics table
...
BigKTS:
    ...
    sta Sprite_Tilenumber+$18,y ; to animate player's feet when swimming
ExPGH:
    rts ;then leave
...
PlayerOffscreenChk:
    ...
    lda SprDataOffset ;get player's sprite data offset
    clc
...
PIntLoop:
    ...
    lda Sprite_Attributes+$24 ;get empty sprite attributes
    ora #%01000000 ;set horizontal flip bit for bottom-right sprite
    sta Sprite_Attributes+$20 ;store and leave
    rts
...
RenderPlayerSub:
    ...
    ldy SprDataOffset ;get player's sprite data offset
DrawPlayerLoop:
...
ChangeSizeOffsetAdder:
    .byte $00,$01,$00,$01,$00,$01,$02,$00,$01,$02,$02,$00,$02,$00,$02,$00
    .byte $02,$00,$02,$00
...
ChkForPlayerAttrib:
    ldy SprDataOffset ;get sprite data offset
    lda GameEngineSubroutine
...
KilledAtt:
    lda Sprite_Attributes+$10,y
    and #%00111111 ;mask out horizontal and vertical flip bits
    sta Sprite_Attributes+$10,y ; for third row sprites and save
    lda Sprite_Attributes+$14,y
    and #%00111111
    ora #%01000000 ;set horizontal flip bit for second
    sta Sprite_Attributes+$14,y ; sprite in the third row
C_S_IGAtt:
    lda Sprite_Attributes+$18,y
    and #%00111111 ;mask out horizontal and vertical flip bits
    sta Sprite_Attributes+$18,y ; for fourth row sprites and save
    lda Sprite_Attributes+$1c,y
    and #%00111111
    ora #%01000000 ;set horizontal flip bit for second
    sta Sprite_Attributes+$1c,y ; sprite in the fourth row
ExPlyrAt:
    rts ;leave
...
XOffscreenBitsData:
    .byte $7f,$3f,$1f,$0f,$07,$03,$01,$00,$80,$c0,$e0,$f0,$f8,$fc,$fe,$ff
DefaultXOnscreenOfs:
    .byte $07,$0f,$07
...
; -----------------------------------------------------------------------------
YOffscreenBitsData:
    .byte $00,$08,$0c,$0e,$0f,$07,$03,$01,$00
DefaultYOnscreenOfs:
    .byte $04,$00,$04
HighPosUnitData:
    .byte $ff,$00
...
SoundEngine:
    lda OperMode ;are we in title screen mode?
    bne SndOn
    sta SND_CHN ;if so, disable sound and leave
    rts

SndOn:
    lda #$ff
    sta JOY2 ;disable irqs and set frame counter mode???
    lda #$0f
    sta SND_CHN ;enable first four channels
...
InPause:
    ...
    lda #$0f
    sta SND_CHN ;enable sound again
...
DecPauC:
    ...
    sta SND_CHN ; not currently playing the pause sfx
...
StrWave:
    sty DMC_RAW ;store into DMC load register (??)
    rts ;we are done here

; -----------------------------------------------------------------------------
Dump_Squ1_Regs:
    sty SQ1_SWEEP ;dump the contents of X and Y into square 1's control regs
    stx SQ1_VOL
    rts
...
Dump_Freq_Regs:
    ...
    sta SQ1_LO,x ;first byte goes into LSB of frequency divider
...
Dump_Sq2_Regs:
    stx SQ2_VOL ;dump the contents of X and Y into square 2's control regs
    sty SQ2_SWEEP
    rts
...
ContinueSwimStomp:
    ...
    sta SQ1_VOL ; envelope
    cpy #$06
    bne BranchToDecLength1
    lda #$9e ;when the length counts down to a certain point, put this
    sta SQ1_LO ; directly into the LSB of square 1's frequency divider
BranchToDecLength1:
...
ContinueSmackEnemy:
    ...
    sta SQ1_LO ; in the smack enemy sound
    lda #$9f
    bne SmTick
...
PowerUpGrabFreqData:
    .byte $4c,$52,$4c,$48,$3e,$36,$3e,$36,$30,$28,$4a,$50,$4a,$64,$3c,$32
    .byte $3c,$32,$2c,$24,$3a,$64,$3a,$34,$2c,$22,$2c
    .byte $22,$1c,$14 ;residual frequency data
PUp_VGrow_FreqData:
...
BrickShatterFreqData:
    .byte $01,$0e,$0e,$0d,$0b,$06,$0c,$0f,$0a,$09,$03,$0d,$08,$0d,$06,$0c
...
NoDecEnv1:
    ...
    sta SQ2_VOL ;based on offset set by first load unless playing
...
FetchSqu1MusicData:
    ...
    lda #$83
    sta SQ1_VOL ;store some data into control regs for square 1
    lda #$94 ; and fetch another byte of data, used to give
    sta SQ1_SWEEP ; death music its unique sound
    sta AltRegContentFlag
    bne FetchSqu1MusicData ;unconditional branch

Squ1NoteHandler:
    jsr AlternateLengthNalder
...
HandleTriangleMusic:
    ...
    lda #$1f
    sta TRI_LINEAR ;load some default data for triangle control reg
...
LoadTriCtrlReg:
    sta TRI_LINEAR ;save final contents of A into control reg for triangle
HandleNoiseMusic:
...
SilentBeat:
    lda #$10 ;silence
PlayBeat:
    sta NOISE_VOL ;load beat data into noise regs
    stx NOISE_LO
    sty NOISE_HI
ExitMusicHandler:
    rts
...
MusicHeaderData:
    .byte $a5,$59,$54,$64,$59,$3c,$31,$4b ;event music
    .byte $69,$5e,$46,$4f,$36,$8d,$36,$4b ;area music
    .byte $8d,$69,$69,$6f,$75,$6f,$7b,$6f,$75,$6f,$7b,$81,$87,$81,$8d,$69 ;ground level music layout
    .byte $69,$93,$99,$93,$9f,$93,$99,$93,$9f,$81,$87,$81,$8d,$93,$99,$93
    .byte $9f
; 
; music headers
...
TimeRunningOutHdr:
    .byte $08
    .byte <TimeRunOutMusData
    .byte >TimeRunOutMusData
    .byte $27,$18
Str_CloudHdr:
    .byte $20
    .byte <Star_CloudMData
    .byte >Star_CloudMData
    .byte $2e,$1a,$40
EndOfLevelMusHdr:
    .byte $20
    .byte <WinLevelMusData
    .byte >WinLevelMusData
    .byte $3d,$21
ResidualHeaderData:
    .byte $20
...
GroundLevelPart2AHdr:
    .byte $18
    .byte <GroundM_P2AData
    .byte >GroundM_P2AData
    .byte $20,$12,$70
GroundLevelPart2BHdr:
    .byte $18
    .byte <GroundM_P2BData
    .byte >GroundM_P2BData
    .byte $1b,$10,$44
GroundLevelPart2CHdr:
    .byte $18
    .byte <GroundM_P2CData
    .byte >GroundM_P2CData
    .byte $11,$0a,$1c
GroundLevelPart3AHdr:
    .byte $18
    .byte <GroundM_P3AData
    .byte >GroundM_P3AData
    .byte $2d,$10,$58
GroundLevelPart3BHdr:
    .byte $18
    .byte <GroundM_P3BData
    .byte >GroundM_P3BData
    .byte $14,$0d,$3f
GroundLevelLeadInHdr:
    .byte $18
    .byte <GroundMLdInData
    .byte >GroundMLdInData
    .byte $15,$0d,$21
GroundLevelPart4AHdr:
    .byte $18
    .byte <GroundM_P4AData
    .byte >GroundM_P4AData
    .byte $18,$10,$7a
GroundLevelPart4BHdr:
    .byte $18
    .byte <GroundM_P4BData
    .byte >GroundM_P4BData
    .byte $19,$0f,$54
GroundLevelPart4CHdr:
    .byte $18
    .byte <GroundM_P4CData
    .byte >GroundM_P4CData
    .byte $1e,$12,$2b
DeathMusHdr:
    .byte $18
...
Star_CloudMData:
    .byte $84,$2c,$2c,$2c,$82,$04,$2c,$04,$85,$2c,$84,$2c,$2c,$2a,$2a,$2a
    .byte $82,$04,$2a,$04,$85,$2a,$84,$2a,$2a,$00
    .byte $1f,$1f,$1f,$98,$1f,$1f,$98,$9e,$98,$1f,$1d,$1d,$1d,$94,$1d,$1d
    .byte $94,$9c,$94,$1d
    .byte $86,$18,$85,$26,$30,$84,$04,$26,$30,$86,$14,$85,$22,$2c,$84,$04
    .byte $22,$2c
    .byte $21,$d0,$c4,$d0,$31,$d0,$c4,$d0,$00
GroundM_P1Data:
    .byte $85,$2c,$22,$1c,$84,$26,$2a,$82,$28,$26,$04,$87,$22,$34,$3a,$82
    .byte $40,$04,$36,$84,$3a,$34,$82,$2c,$30,$85,$2a
SilenceData:
    .byte $00
    .byte $5d,$55,$4d,$15,$19,$96,$15,$d5,$e3,$eb,$2d,$a6,$2b,$27,$9c,$9e
    .byte $59,$85,$22,$1c,$14,$84,$1e,$22,$82,$20,$1e,$04,$87,$1c,$2c,$34
    .byte $82,$36,$04,$30,$34,$04,$2c,$04,$26,$2a,$85,$22
GroundM_P2AData:
    .byte $84,$04,$82,$3a,$38,$36,$32,$04,$34,$04,$24,$26,$2c,$04,$26,$2c
    .byte $30,$00,$05,$b4,$b2,$b0,$2b,$ac,$84,$9c,$9e,$a2,$84,$94,$9c,$9e
    .byte $85,$14,$22,$84,$2c,$85,$1e,$82,$2c,$84,$2c,$1e
GroundM_P2BData:
    .byte $84,$04,$82,$3a,$38,$36,$32,$04,$34,$04,$64,$04,$64,$86,$64,$00
    .byte $05,$b4,$b2,$b0,$2b,$ac,$84,$37,$b6,$b6,$45,$85,$14,$1c,$82,$22
    .byte $84,$2c,$4e,$82,$4e,$84,$4e,$22
GroundM_P2CData:
    .byte $84,$04,$85,$32,$85,$30,$86,$2c,$04,$00,$05,$a4,$05,$9e,$05,$9d
    .byte $85,$84,$14,$85,$24,$28,$2c,$82,$22,$84,$22,$14,$21,$d0,$c4,$d0
    .byte $31,$d0,$c4,$d0,$00
GroundM_P3AData:
    .byte $82,$2c,$84,$2c,$2c,$82,$2c,$30,$04,$34,$2c,$04,$26,$86,$22,$00
    .byte $a4,$25,$25,$a4,$29,$a2,$1d,$9c,$95
GroundM_P3BData:
    .byte $82,$2c,$2c,$04,$2c,$04,$2c,$30,$85,$34,$04,$04,$00,$a4,$25,$25
    .byte $a4,$a8,$63,$04
; triangle data used by both sections of third part
    .byte $85,$0e,$1a,$84,$24,$85,$22,$14,$84,$0c
GroundMLdInData:
    .byte $82,$34,$84,$34,$34,$82,$2c,$84,$34,$86,$3a,$04,$00,$a0,$21,$21
    .byte $a0,$21,$2b,$05,$a3,$82,$18,$84,$18,$18,$82,$18,$18,$04,$86,$3a
    .byte $22
; noise data used by lead-in and third part sections
    .byte $31,$90,$31,$90,$31,$71,$31,$90,$90,$90,$00
GroundM_P4AData:
    .byte $82,$34,$84,$2c,$85,$22,$84,$24,$82,$26,$36,$04,$36,$86,$26,$00
    .byte $ac,$27,$5d,$1d,$9e,$2d,$ac,$9f,$85,$14,$82,$20,$84,$22,$2c,$1e
    .byte $1e,$82,$2c,$2c,$1e,$04
GroundM_P4BData:
    .byte $87,$2a,$40,$40,$40,$3a,$36,$82,$34,$2c,$04,$26,$86,$22,$00,$e3
    .byte $f7,$f7,$f7,$f5,$f1,$ac,$27,$9e,$9d,$85,$18,$82,$1e,$84,$22,$2a
...
CastleMusData:
    ...
    .byte $74
    .byte $84,$1a,$83,$18,$20,$84,$1e,$83,$1c,$28,$26,$1c,$1a,$1c
GameOverMusData:
    .byte $82,$2c,$04,$04,$22,$04,$04,$84,$1c,$87,$26,$2a,$26,$84,$24,$28
    .byte $24,$80,$22,$00,$9c,$05,$94,$05,$0d,$9f,$1e,$9c,$98,$9d,$82,$22
    .byte $04,$04,$1c,$04,$04,$84,$14,$86,$1e,$80,$16,$80,$14
TimeRunOutMusData:
    .byte $81,$1c,$30,$04,$30,$30,$04,$1e,$32,$04,$32,$32,$04,$20,$34,$04
    .byte $34,$34,$04,$36,$04,$84,$36,$00,$46,$a4,$64,$a4,$48,$a6,$66,$a6
    .byte $4a,$a8,$68,$a8,$6a,$44,$2b,$81,$2a,$42,$04,$42,$42,$04,$2c,$64
    .byte $04,$64,$64,$04,$2e,$46,$04,$46,$46,$04,$22,$04,$84,$22
WinLevelMusData:
    .byte $87,$04,$06,$0c,$14,$1c,$22,$86,$2c,$22,$87,$04,$60,$0e,$14,$1a
    .byte $24,$86,$2c,$24,$87,$04,$08,$10,$18,$1e,$28,$86,$30,$30,$80,$64
    .byte $00,$cd,$d5,$dd,$e3,$ed,$f5,$bb,$b5,$cf,$d5,$db,$e5,$ed,$f3,$bd
    .byte $b3,$d1,$d9,$df,$e9,$f1,$f7,$bf,$ff,$ff,$ff,$34
    .byte $00 ;unused byte
    .byte $86,$04,$87,$14,$1c,$22,$86,$34,$84,$2c,$04,$04,$04,$87,$14,$1a
    .byte $24,$86,$32,$84,$2c,$04,$86,$04,$87,$18,$1e,$28,$86,$36,$87,$30
    .byte $30,$30,$80,$2c
; square 2 and triangle use the same data, square 1 is unused
UndergroundMusData:
    .byte $82,$14,$2c,$62,$26,$10,$28,$80,$04,$82,$14,$2c,$62,$26,$10,$28
    .byte $80,$04,$82,$08,$1e,$5e,$18,$60,$1a,$80,$04,$82,$08,$1e,$5e,$18
    .byte $60,$1a,$86,$04,$83,$1a,$18,$16,$84,$14,$1a,$18,$0e,$0c,$16,$83
    .byte $14,$20,$1e,$1c,$28,$26,$87,$24,$1a,$12,$10,$62,$0e,$80,$04,$04
    .byte $00
; noise data directly follows square 2 here unlike in other songs
WaterMusData:
    .byte $82,$18,$1c,$20,$22,$26,$28,$81,$2a,$2a,$2a,$04,$2a,$04,$83,$2a
    .byte $82,$22,$86,$34,$32,$34,$81,$04,$22,$26,$2a,$2c,$30,$86,$34,$83
    .byte $32,$82,$36,$84,$34,$85,$04,$81,$22,$86,$30,$2e,$30,$81,$04,$22
    .byte $26,$2a,$2c,$2e,$86,$30,$83,$22,$82,$36,$84,$34,$85,$04,$81,$22
    .byte $86,$3a,$3a,$3a,$82,$3a,$81,$40,$82,$04,$81,$3a,$86,$36,$36,$36
    .byte $82,$36,$81,$3a,$82,$04,$81,$36,$86,$34,$82,$26,$2a,$36,$81,$34
    .byte $34,$85,$34,$81,$2a,$86,$2c,$00
    .byte $84,$90,$b0,$84,$50,$50,$b0,$00
    .byte $98,$96,$94,$92,$94,$96,$58,$58,$58,$44,$5c,$44,$9f,$a3,$a1,$a3
    .byte $85,$a3,$e0,$a6,$23,$c4,$9f,$9d,$9f,$85,$9f,$d2,$a6,$23,$c4,$b5
    .byte $b1,$af,$85,$b1,$af,$ad,$85,$95,$9e,$a2,$aa,$6a,$6a,$6b,$5e,$9d
    .byte $84,$04,$04,$82,$22,$86,$22,$82,$14,$22,$2c,$12,$22,$2a,$14,$22
    .byte $2c,$1c,$22,$2c,$14,$22,$2c,$12,$22,$2a,$14,$22,$2c,$1c,$22,$2c
    .byte $18,$22,$2a,$16,$20,$28,$18,$22,$2a,$12,$22,$2a,$18,$22,$2a,$12
    .byte $22,$2a,$14,$22,$2c,$0c,$22,$2c,$14,$22,$34,$12,$22,$30,$10,$22
    .byte $2e,$16,$22,$34,$18,$26,$36,$16,$26,$36,$14,$26,$36,$12,$22,$36
    .byte $5c,$22,$34,$0c,$22,$22,$81,$1e,$1e,$85,$1e,$81,$12,$86,$14
EndOfCastleMusData:
    .byte $81,$2c,$22,$1c,$2c,$22,$1c,$85,$2c,$04,$81,$2e,$24,$1e,$2e,$24
    .byte $1e,$85,$2e,$04,$81,$32,$28,$22,$32,$28,$22,$85,$32,$87,$36,$36
    .byte $36,$84,$3a,$00
    ...
    .byte $86,$14,$81,$14,$80,$14,$14,$81,$14,$14,$14,$14,$86,$16,$81,$16
    .byte $80,$16,$16,$81,$16,$16,$16,$16,$81,$28,$22,$1a,$28,$22,$1a,$28
    .byte $80,$28,$28,$81,$28,$87,$2c,$2c,$2c,$84,$30
VictoryMusData:
    .byte $83,$04,$84,$0c,$83,$62,$10,$84,$12,$83,$1c,$22,$1e,$22,$26,$18
    .byte $1e,$04,$1c,$00
    .byte $e3,$e1,$e3,$1d,$de,$e0,$23,$ec,$75,$74,$f0,$f4,$f6,$ea,$31,$2d
    .byte $83,$12,$14,$04,$18,$1a,$1c,$14,$26,$22,$1e,$1c,$18,$1e,$22,$0c
    .byte $14
; unused space
    .byte $ff,$ff,$ff
FreqRegLookupTbl:
    .byte $00,$88,$00,$2f,$00,$00,$02,$a6,$02,$80,$02,$5c,$02,$3a,$02,$1a
    .byte $01,$df,$01,$c4,$01,$ab,$01,$93,$01,$7c,$01,$67,$01,$53,$01,$40
    .byte $01,$2e,$01,$1d,$01,$0d,$00,$fe,$00,$ef,$00,$e2,$00,$d5,$00,$c9
    .byte $00,$be,$00,$b3,$00,$a9,$00,$a0,$00,$97,$00,$8e,$00,$86,$00,$77
    .byte $00,$7e,$00,$71,$00,$54,$00,$64,$00,$5f,$00,$59,$00,$50,$00,$47
    .byte $00,$43,$00,$3b,$00,$35,$00,$2a,$00,$23,$04,$75,$03,$57,$02,$f9
    .byte $02,$cf,$01,$fc,$00,$6a
MusicLengthLookupTbl:
    .byte $05,$0a,$14,$28,$50,$1e,$3c,$02,$04,$08,$10,$20,$40,$18,$30,$0c
    .byte $03,$06,$0c,$18,$30,$12,$24,$08,$36,$03,$09,$06,$12,$1b,$24,$0c
    .byte $24,$02,$06,$04,$0c,$12,$18,$08,$12,$01,$03,$02,$06,$09,$0c,$04
...
BowserFlameEnvData:
    .byte $15,$16,$16,$17,$17,$18,$19,$19,$1a,$1a,$1c,$1d,$1d,$1e,$1e,$1f
    .byte $1f,$1f,$1f,$1e,$1d,$1c,$1e,$1f,$1f,$1e,$1d,$1c,$1a,$18,$16,$14
BrickShatterEnvData:
    .byte $15,$16,$16,$17,$17,$18,$19,$19,$1a,$1a,$1c,$1d,$1d,$1e,$1e,$1f
; 
    .word NonMaskableInterrupt ;NMI vector
    .word Start ;reset vector
    .word BrickShatterEnvData+6 ;IRQ vector (unused)
; 
...
CHR_ROM:
    ...
    .byte $00,$00,$00,$7e,$7e,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00
    .byte $00,$00,$44,$28,$10,$28,$44,$00,$00,$00,$00,$00,$00,$00,$00,$00
    .byte $ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$7f,$7f,$7f,$7f,$7f,$7f,$7f,$7f
    .byte $18,$3c,$3c,$3c,$18,$18,$00,$18,$00,$00,$00,$00,$00,$00,$00,$00
    .byte $ff,$7f,$7f,$7f,$7f,$ff,$e3,$c1,$ff,$80,$80,$80,$80,$00,$1c,$3e
    .byte $80,$80,$80,$c1,$e3,$ff,$ff,$ff,$7f,$7f,$7f,$3e,$1c,$00,$00,$ff
    .byte $38,$7c,$7c,$7c,$7c,$7c,$38,$00,$08,$04,$04,$04,$04,$04,$08,$00
    .byte $03,$06,$0c,$0c,$08,$08,$04,$03,$03,$05,$0b,$0b,$0f,$0f,$07,$03
    .byte $01,$02,$04,$08,$10,$20,$40,$80,$01,$03,$07,$0f,$1f,$3f,$7f,$ff
    .byte $00,$00,$00,$00,$00,$07,$38,$c0,$00,$00,$00,$00,$00,$07,$3f,$ff
    .byte $00,$00,$00,$00,$00,$e0,$1c,$03,$00,$00,$00,$00,$00,$e0,$fc,$ff
    .byte $80,$40,$20,$10,$08,$04,$02,$01,$80,$c0,$e0,$f0,$f8,$fc,$fe,$ff
    .byte $04,$0e,$0e,$0e,$6e,$64,$60,$60,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff
    .byte $07,$0f,$1f,$1f,$7f,$ff,$ff,$7f,$07,$08,$10,$00,$60,$80,$80,$40
    .byte $03,$07,$1f,$3f,$3f,$3f,$79,$f7,$03,$04,$18,$20,$20,$20,$46,$88
    .byte $c0,$e0,$f0,$f4,$fe,$bf,$df,$ff,$c0,$20,$10,$14,$0a,$41,$21,$01
    ...
    .byte $c0,$20,$10,$10,$10,$10,$20,$c0,$c0,$e0,$f0,$f0,$f0,$f0,$e0,$c0
    .byte $00,$00,$00,$00,$3f,$7f,$e0,$c0,$00,$00,$00,$00,$00,$00,$1c,$3e
    .byte $88,$9c,$88,$80,$80,$80,$80,$80,$7f,$7f,$7f,$3e,$1c,$00,$00,$00
    .byte $fe,$fe,$fe,$fe,$fe,$fe,$fe,$fe,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff
    .byte $08,$14,$24,$c4,$03,$40,$a1,$26,$00,$08,$18,$38,$fc,$bf,$5e,$d9
    .byte $ff,$ff,$ff,$ff,$7f,$7f,$7f,$7f,$81,$81,$81,$81,$81,$81,$81,$81
    ...
    .byte $3f,$7f,$7f,$ff,$ff,$ff,$ff,$ff,$3f,$60,$40,$c0,$80,$80,$80,$80
    .byte $ff,$ff,$ff,$ff,$ff,$ff,$7e,$3c,$80,$80,$80,$80,$80,$81,$42,$3c
    .byte $ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$00,$00,$00,$00,$00,$00,$00
    .byte $ff,$ff,$ff,$ff,$ff,$ff,$fe,$7c,$00,$00,$00,$00,$00,$01,$82,$7c
    .byte $ff,$ff,$ff,$ff,$ff,$ff,$fe,$7c,$00,$00,$00,$00,$00,$01,$83,$ff
    .byte $f8,$fc,$fe,$fe,$ff,$ff,$ff,$ff,$f8,$04,$02,$02,$01,$01,$01,$01
    .byte $ff,$ff,$ff,$ff,$ff,$ff,$7e,$3c,$01,$01,$01,$01,$01,$81,$42,$3c
    .byte $00,$08,$08,$08,$10,$10,$10,$00,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff
    ...
    .byte $7f,$80,$a0,$80,$80,$80,$80,$80,$7f,$ff,$ff,$ff,$ff,$ff,$ff,$ff
    .byte $fe,$01,$05,$01,$01,$01,$01,$01,$fe,$ff,$ff,$ff,$ff,$ff,$ff,$ff
    .byte $80,$80,$80,$80,$80,$a0,$80,$7f,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$7f
    .byte $01,$01,$01,$01,$01,$05,$01,$fe,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$fe
    .byte $00,$00,$00,$00,$fc,$fe,$07,$03,$00,$00,$00,$00,$00,$00,$38,$7c
    .byte $11,$39,$11,$01,$01,$01,$01,$01,$fe,$fe,$fe,$7c,$38,$00,$00,$00
    .byte $ef,$28,$28,$28,$28,$28,$ef,$00,$20,$e7,$e7,$e7,$e7,$e7,$ef,$00
    .byte $fe,$82,$82,$82,$82,$82,$fe,$00,$02,$7e,$7e,$7e,$7e,$7e,$fe,$00
    .byte $80,$80,$80,$98,$9c,$8c,$80,$7f,$7f,$7f,$7f,$67,$67,$7f,$7f,$7f
    .byte $ff,$ff,$83,$f3,$f3,$f3,$f3,$f3,$ff,$80,$fc,$8c,$8c,$8c,$8c,$8c
    .byte $ff,$ff,$f0,$f6,$f6,$f6,$f6,$f6,$ff,$00,$0f,$09,$09,$09,$09,$09
    .byte $ff,$ff,$00,$00,$00,$00,$00,$00,$ff,$00,$ff,$ff,$ff,$ff,$ff,$ff
    .byte $ff,$ff,$01,$57,$2f,$57,$2f,$57,$ff,$01,$ff,$a9,$d1,$a9,$d1,$a9
    .byte $f3,$f3,$f3,$f3,$f3,$f3,$ff,$3f,$8c,$8c,$8c,$8c,$8c,$8c,$ff,$3f
    .byte $f6,$f6,$f6,$f6,$f6,$f6,$ff,$ff,$09,$09,$09,$09,$09,$09,$ff,$ff
    .byte $00,$00,$00,$00,$00,$00,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff
    .byte $2f,$57,$2f,$57,$2f,$57,$ff,$fc,$d1,$a9,$d1,$a9,$d1,$a9,$ff,$fc
    .byte $3c,$3c,$3c,$3c,$3c,$3c,$3c,$3c,$23,$23,$23,$23,$23,$23,$23,$23
    .byte $fb,$fb,$fb,$fb,$fb,$fb,$fb,$fb,$04,$04,$04,$04,$04,$04,$04,$04
    .byte $bc,$5c,$bc,$5c,$bc,$5c,$bc,$5c,$44,$a4,$44,$a4,$44,$a4,$44,$a4
    .byte $1f,$20,$40,$40,$80,$80,$80,$81,$1f,$3f,$7f,$7f,$ff,$ff,$ff,$fe
    .byte $ff,$80,$80,$c0,$ff,$ff,$fe,$fe,$ff,$7f,$7f,$3f,$00,$00,$01,$01
    .byte $ff,$7f,$7f,$ff,$ff,$07,$03,$03,$ff,$80,$80,$00,$00,$f8,$fc,$fc
    .byte $ff,$00,$00,$00,$00,$81,$c3,$ff,$ff,$ff,$ff,$ff,$ff,$7e,$3c,$00
    .byte $f8,$fc,$fe,$fe,$e3,$c1,$81,$81,$f8,$04,$02,$02,$1d,$3f,$7f,$7f
    .byte $83,$ff,$ff,$ff,$ff,$ff,$7f,$1f,$fc,$80,$80,$80,$80,$80,$60,$1f
    .byte $fc,$fc,$fc,$fc,$fe,$fe,$ff,$ff,$03,$03,$03,$03,$01,$01,$00,$ff
    .byte $01,$01,$01,$01,$03,$03,$07,$ff,$fe,$fe,$fe,$fe,$fc,$fc,$f8,$ff
    .byte $ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$00,$00,$00,$00,$00,$00,$00,$ff
    .byte $81,$c1,$e3,$ff,$ff,$ff,$ff,$fe,$7f,$3f,$1d,$01,$01,$01,$03,$fe
    .byte $ff,$ff,$ff,$ff,$ff,$fb,$b5,$ce,$80,$80,$80,$80,$80,$84,$ca,$b1
    .byte $ff,$ff,$ff,$ff,$ff,$df,$ad,$73,$01,$01,$01,$01,$01,$21,$53,$8d
    .byte $77,$77,$77,$77,$77,$77,$77,$77,$00,$00,$00,$00,$77,$ff,$ff,$ff
    .byte $00,$00,$00,$00,$00,$00,$00,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff
    .byte $77,$77,$77,$77,$00,$00,$00,$00,$ff,$ff,$ff,$77,$77,$77,$77,$77
    .byte $01,$01,$01,$19,$1d,$0d,$01,$fe,$ff,$ff,$ff,$e7,$e7,$ff,$ff,$fe
    .byte $20,$78,$7f,$fe,$fe,$fe,$fe,$fe,$00,$21,$21,$41,$41,$41,$41,$41
    .byte $04,$9a,$fa,$fd,$fd,$fd,$fd,$fd,$00,$80,$80,$80,$80,$80,$80,$80
    .byte $7e,$38,$21,$00,$01,$00,$01,$00,$21,$21,$01,$01,$01,$01,$01,$01
    .byte $fa,$8a,$84,$80,$80,$80,$80,$80,$80,$80,$80,$80,$80,$80,$80,$80
    .byte $02,$04,$00,$10,$00,$40,$80,$00,$01,$01,$06,$08,$18,$20,$20,$c0
    .byte $0b,$0b,$3b,$0b,$fb,$0b,$0b,$0a,$04,$04,$c4,$f4,$f4,$04,$04,$05
    .byte $90,$10,$1f,$10,$1f,$10,$10,$90,$70,$f0,$f0,$ff,$ff,$f0,$f0,$70
    .byte $3f,$78,$e7,$cf,$58,$58,$50,$90,$c0,$87,$18,$b0,$e7,$e7,$ef,$ef
    .byte $b0,$fc,$e2,$c1,$c1,$83,$8f,$7e,$6f,$43,$5d,$3f,$3f,$7f,$7f,$ff
    .byte $fe,$03,$0f,$91,$70,$60,$20,$31,$03,$ff,$f1,$6e,$cf,$df,$ff,$ff
    .byte $3f,$3f,$1d,$39,$7b,$f3,$86,$fe,$fd,$fb,$fb,$f7,$f7,$0f,$7f,$ff
    .byte $ff,$ff,$ff,$ff,$ff,$80,$80,$ff,$ff,$80,$80,$80,$80,$ff,$ff,$80
    .byte $fe,$ff,$ff,$ff,$ff,$03,$03,$ff,$fe,$03,$03,$03,$03,$ff,$ff,$03
    .byte $00,$ff,$ff,$ff,$ff,$ff,$00,$00,$00,$ff,$00,$00,$00,$00,$ff,$ff
    .byte $3c,$fc,$fc,$fc,$fc,$fc,$04,$04,$23,$f3,$0b,$0b,$0b,$07,$ff,$ff
    .byte $ff,$ff,$ff,$ff,$80,$ff,$ff,$ff,$80,$80,$80,$80,$ff,$80,$80,$80
    .byte $ff,$ff,$ff,$ff,$03,$ff,$ff,$ff,$03,$03,$03,$03,$ff,$03,$03,$03
    .byte $ff,$ff,$ff,$ff,$ff,$00,$ff,$ff,$00,$00,$00,$00,$00,$ff,$00,$00
    .byte $fc,$fc,$fe,$fe,$fe,$02,$fe,$fe,$07,$07,$03,$03,$03,$ff,$03,$03
    .byte $ff,$80,$80,$80,$80,$80,$80,$80,$80,$ff,$ff,$ff,$ff,$ff,$ff,$ff
    .byte $ff,$03,$03,$03,$03,$03,$03,$03,$03,$ff,$ff,$ff,$ff,$ff,$ff,$ff
    .byte $02,$02,$02,$02,$02,$02,$04,$04,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff
    .byte $80,$80,$aa,$d5,$aa,$ff,$ff,$ff,$ff,$ff,$d5,$aa,$d5,$80,$80,$ff
    .byte $03,$03,$ab,$57,$ab,$ff,$ff,$fe,$ff,$ff,$57,$ab,$57,$03,$03,$fe
    .byte $00,$55,$aa,$55,$ff,$ff,$ff,$00,$ff,$aa,$55,$aa,$00,$00,$ff,$00
    .byte $04,$54,$ac,$5c,$fc,$fc,$fc,$3c,$ff,$af,$57,$ab,$0b,$0b,$f3,$23
    .byte $3f,$3f,$3f,$3f,$00,$00,$00,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff
    .byte $7e,$7c,$7c,$78,$00,$00,$00,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff
    .byte $1f,$0f,$0f,$07,$00,$00,$00,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff
    .byte $fe,$fc,$fc,$f8,$00,$00,$00,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff
    .byte $00,$00,$00,$00,$ff,$ff,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00
    .byte $18,$18,$18,$18,$18,$18,$18,$18,$00,$00,$00,$00,$00,$00,$00,$00
    .byte $07,$1f,$3f,$ff,$7f,$7f,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff
    .byte $e1,$f9,$fd,$ff,$fe,$fe,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff
    .byte $f0,$10,$10,$10,$10,$10,$10,$ff,$00,$e0,$e0,$e0,$e0,$e0,$e0,$e0
    .byte $1f,$10,$10,$10,$10,$10,$10,$ff,$00,$0f,$0f,$0f,$0f,$0f,$0f,$0f
    .byte $92,$92,$92,$fe,$fe,$00,$00,$00,$48,$48,$6c,$00,$00,$00,$fe,$00
    .byte $0a,$0a,$3a,$0a,$fb,$0b,$0b,$0b,$05,$05,$c5,$f5,$f4,$04,$04,$04
    .byte $90,$90,$9f,$90,$9f,$90,$90,$90,$70,$70,$70,$7f,$7f,$70,$70,$70
    .byte $01,$01,$01,$01,$01,$01,$01,$01,$00,$00,$00,$00,$00,$00,$00,$00
    .byte $80,$80,$80,$80,$80,$80,$80,$80,$00,$00,$00,$00,$00,$00,$00,$00
    .byte $08,$88,$91,$d1,$53,$53,$73,$3f,$ff,$ff,$ff,$ff,$ff,$fe,$be,$ce
    .byte $00,$00,$07,$0f,$0c,$1b,$1b,$1b,$00,$00,$00,$00,$03,$04,$04,$04
    .byte $00,$00,$e0,$f0,$f0,$f8,$f8,$f8,$00,$00,$60,$30,$30,$98,$98,$98
    .byte $1b,$1b,$1b,$1b,$1b,$0f,$0f,$07,$04,$04,$04,$04,$04,$03,$00,$00
    .byte $f8,$f8,$f8,$f8,$f8,$f0,$f0,$e0,$98,$98,$98,$98,$98,$30,$30,$60
    .byte $f1,$11,$11,$1f,$10,$10,$10,$ff,$0f,$ef,$ef,$ef,$ef,$ef,$ef,$e0
    .byte $1f,$10,$10,$f0,$10,$10,$10,$ff,$e0,$ef,$ef,$ef,$ef,$ef,$ef,$0f
    .byte $7f,$bf,$df,$ef,$f0,$f0,$f0,$f0,$80,$40,$20,$10,$0f,$0f,$0f,$0f
    .byte $f0,$f0,$f0,$f0,$ff,$ff,$ff,$ff,$0f,$0f,$0f,$0f,$1f,$3f,$7f,$ff
    .byte $ff,$ff,$ff,$ff,$0f,$0f,$0f,$0f,$01,$03,$07,$0f,$ff,$ff,$ff,$ff
    .byte $0f,$0f,$0f,$0f,$f7,$fb,$fd,$fe,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff
    .byte $00,$00,$00,$00,$00,$00,$18,$18,$00,$00,$00,$00,$00,$00,$00,$00
    .byte $1f,$3f,$7f,$7f,$7f,$ff,$ff,$ff,$1f,$20,$40,$40,$40,$80,$82,$82
    .byte $ff,$ff,$ff,$7f,$7f,$7f,$3f,$1e,$82,$80,$a0,$44,$43,$40,$21,$1e
    .byte $f8,$fc,$fe,$fe,$fe,$ff,$ff,$ff,$f8,$04,$02,$02,$02,$01,$41,$41
    .byte $ff,$ff,$ff,$fe,$fe,$fe,$fc,$78,$41,$01,$05,$22,$c2,$02,$84,$78
    .byte $7f,$80,$80,$80,$80,$80,$80,$80,$80,$7f,$7f,$7f,$7f,$7f,$7f,$7f
    .byte $de,$61,$61,$61,$71,$5e,$7f,$61,$61,$df,$df,$df,$df,$ff,$c1,$df
    .byte $80,$80,$c0,$f0,$bf,$8f,$81,$7e,$7f,$7f,$ff,$3f,$4f,$71,$7f,$ff
    .byte $61,$61,$c1,$c1,$81,$81,$83,$fe,$df,$df,$bf,$bf,$7f,$7f,$7f,$7f
    .byte $00,$00,$03,$0f,$1f,$3f,$7f,$7f,$00,$00,$03,$0c,$10,$20,$40,$40
    .byte $00,$00,$c0,$f0,$f8,$fc,$fe,$fe,$00,$00,$c0,$30,$08,$04,$02,$02
    .byte $ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$80,$80,$80,$80,$80,$80,$80,$80
    .byte $ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$01,$01,$01,$01,$01,$01,$01,$01
    .byte $7f,$7f,$7f,$3f,$3f,$1f,$0f,$07,$40,$40,$40,$20,$30,$1c,$0f,$07
    .byte $fe,$fe,$fe,$fc,$fc,$f8,$f0,$f0,$02,$02,$02,$04,$0c,$38,$f0,$f0
    .byte $0f,$0f,$0f,$0f,$0f,$0f,$07,$0f,$08,$08,$08,$08,$08,$0c,$05,$0a
    .byte $f0,$f0,$f0,$f0,$f0,$f0,$e0,$f0,$10,$50,$50,$50,$50,$30,$a0,$50
    .byte $81,$c1,$a3,$a3,$9d,$81,$81,$81,$00,$41,$22,$22,$1c,$00,$00,$00
    .byte $e3,$f7,$c1,$c1,$c1,$c1,$f7,$e3,$e3,$14,$3e,$3e,$3e,$3e,$14,$e3
    .byte $00,$00,$07,$0f,$0c,$1b,$1b,$1b,$ff,$ff,$f8,$f0,$f0,$e0,$e0,$e0
    .byte $00,$00,$e0,$f0,$f0,$f8,$f8,$f8,$ff,$ff,$7f,$3f,$3f,$9f,$9f,$9f
    .byte $1b,$1b,$1b,$1b,$1b,$0f,$0f,$07,$e0,$e0,$e0,$e0,$e0,$f3,$f0,$f8
    .byte $f8,$f8,$f8,$f8,$f8,$f0,$f0,$e0,$9f,$9f,$9f,$9f,$9f,$3f,$3f,$7f
    ...
    .byte $00,$00,$00,$00,$00,$00,$7e,$3c,$3c,$7e,$7e,$ff,$ff,$ff,$42,$00
    .byte $3c,$42,$99,$a1,$a1,$99,$42,$3c,$00,$00,$00,$00,$00,$00,$00,$00
    .byte $0f,$1f,$1f,$3f,$3f,$7f,$7f,$7f,$f0,$e0,$e0,$c0,$c0,$80,$80,$80
    .byte $f0,$f8,$f8,$fc,$fc,$fe,$fe,$fe,$0f,$07,$07,$03,$03,$01,$01,$01
    .byte $7f,$7f,$3f,$3f,$3f,$3f,$1f,$1f,$80,$80,$c0,$c0,$e0,$f8,$fe,$ff
    .byte $fe,$ff,$ff,$ff,$fc,$fc,$fe,$fe,$ff,$7f,$1f,$07,$03,$03,$01,$81
    .byte $7f,$7f,$7f,$3f,$3f,$3f,$3f,$1f,$80,$80,$80,$c0,$c0,$e0,$e0,$f0
    .byte $fe,$fe,$ff,$ff,$ff,$ff,$ff,$fe,$01,$01,$01,$03,$03,$07,$07,$0f
    .byte $1f,$0f,$0f,$07,$00,$00,$00,$00,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff
    .byte $fe,$fc,$fc,$f8,$00,$00,$00,$00,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff
    .byte $7e,$7e,$7e,$7e,$7f,$7f,$7f,$7f,$81,$81,$81,$81,$81,$81,$81,$81
    .byte $ff,$ff,$ff,$ff,$ff,$ff,$ff,$fe,$01,$01,$01,$03,$03,$07,$07,$0f
    .byte $fe,$fe,$fe,$fe,$ff,$ff,$ff,$ff,$01,$01,$01,$01,$01,$01,$01,$01
    .byte $7f,$7f,$7f,$7f,$7f,$7f,$7f,$7f,$81,$81,$81,$81,$81,$81,$81,$81
    .byte $ff,$ff,$ff,$ff,$fc,$fe,$fe,$7e,$ff,$03,$03,$03,$03,$03,$03,$ff
    .byte $ff,$ff,$ff,$ff,$00,$00,$00,$00,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff
    .byte $7f,$7f,$7f,$7f,$7f,$7f,$7f,$7f,$80,$80,$80,$80,$80,$80,$80,$80
    .byte $ff,$ff,$ff,$ff,$ff,$ff,$ff,$fe,$01,$01,$01,$03,$07,$03,$01,$01
    .byte $7e,$7e,$7f,$7f,$7f,$7f,$7f,$7f,$81,$81,$81,$81,$81,$81,$81,$81
    .byte $3f,$3f,$3f,$3f,$00,$00,$00,$00,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff
    .byte $7e,$7c,$7c,$78,$00,$00,$00,$00,$ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff
    .byte $fe,$fe,$ff,$ff,$7f,$7f,$7f,$7f,$81,$81,$81,$81,$81,$81,$81,$81
    .byte $7f,$7f,$3f,$3f,$3f,$3f,$1f,$1f,$80,$80,$c0,$c0,$e0,$f8,$fe,$ff
    .byte $3f,$bf,$ff,$ff,$fc,$fc,$fe,$fe,$ff,$7f,$1f,$07,$03,$03,$01,$81
    .byte $7f,$7f,$7e,$7e,$7f,$7f,$7f,$7f,$81,$81,$81,$81,$81,$81,$81,$81
    .byte $7e,$7e,$7e,$7e,$7f,$7f,$7f,$7f,$81,$81,$81,$81,$81,$81,$81,$81
    .byte $81,$c3,$c3,$e7,$e7,$ff,$ff,$ff,$7e,$3c,$3c,$18,$18,$00,$00,$00
    .byte $0f,$43,$5b,$53,$31,$19,$0f,$07,$f2,$fe,$fe,$ff,$ff,$ef,$f7,$f8
    .byte $c1,$c3,$c6,$84,$fc,$fc,$0e,$02,$bf,$be,$bd,$7b,$7b,$07,$f3,$fd
    .byte $10,$20,$22,$ba,$e6,$e1,$c0,$c0,$ff,$ff,$ff,$67,$59,$9e,$bf,$bf
    .byte $20,$a6,$54,$26,$20,$c6,$54,$26,$20,$e6,$54,$26,$21,$06,$54,$26
    .byte $20,$85,$01,$44,$20,$86,$54,$48,$20,$9a,$01,$49,$20,$a5,$c9,$46
    .byte $20,$ba,$c9,$4a,$20,$a6,$0a,$d0,$d1,$d8,$d8,$de,$d1,$d0,$da,$de
    .byte $d1,$20,$c6,$0a,$d2,$d3,$db,$db,$db,$d9,$db,$dc,$db,$df,$20,$e6
    .byte $0a,$d4,$d5,$d4,$d9,$db,$e2,$d4,$da,$db,$e0,$21,$06,$0a,$d6,$d7
    .byte $d6,$d7,$e1,$26,$d6,$dd,$e1,$e1,$21,$26,$14,$d0,$e8,$d1,$d0,$d1
    .byte $de,$d1,$d8,$d0,$d1,$26,$de,$d1,$de,$d1,$d0,$d1,$d0,$d1,$26,$21
    .byte $46,$14,$db,$42,$42,$db,$42,$db,$42,$db,$db,$42,$26,$db,$42,$db
    .byte $42,$db,$42,$db,$42,$26,$21,$66,$46,$db,$21,$6c,$0e,$df,$db,$db
    .byte $db,$26,$db,$df,$db,$df,$db,$db,$e4,$e5,$26,$21,$86,$14,$db,$db
    .byte $db,$de,$43,$db,$e0,$db,$db,$db,$26,$db,$e3,$db,$e0,$db,$db,$e6
    .byte $e3,$26,$21,$a6,$14,$db,$db,$db,$db,$42,$db,$db,$db,$d4,$d9,$26
    .byte $db,$d9,$db,$db,$d4,$d9,$d4,$d9,$e7,$21,$c5,$16,$5f,$95,$95,$95
    .byte $95,$95,$95,$95,$95,$97,$98,$78,$95,$96,$95,$95,$97,$98,$97,$98
    .byte $95,$7a,$21,$ed,$0e,$cf,$01,$09,$08,$05,$24,$17,$12,$17,$1d,$0e
    .byte $17,$0d,$18,$22,$4b,$0d,$01,$24,$19,$15,$0a,$22,$0e,$1b,$24,$10
    .byte $0a,$16,$0e,$22,$8b,$0d,$02,$24,$19,$15,$0a,$22,$0e,$1b,$24,$10
    .byte $0a,$16,$0e,$22,$ec,$04,$1d,$18,$19,$28,$22,$f6,$01,$00,$23,$c9
    .byte $56,$55,$23,$e2,$04,$99,$aa,$aa,$aa,$23,$ea,$04,$99,$aa,$aa,$aa
